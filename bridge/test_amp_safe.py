...
class AmpSafeTest(unittest.TestCase):
    ...
    def test_amp_uses_native_credentials_without_a1_or_profile_rewriting(self):
        overrides = {
            "AMP_BUC_TOKEN": "stale-buc-token",
            "AMP_PRIVATE_TOKEN": "ambient-pat",
            "AMP_HOME": "/tmp/untrusted-amp",
            "AMP_PROFILE": "other",
            "A1ID_ROOT": "/tmp/untrusted-a1",
            "JARVIS_CODE_AUTH_FILE": "/tmp/untrusted-auth",
            "JARVIS_CODE_ASKPASS_MODE": "1",
        }
        self.environment.update(overrides)
        self.environment.update({"HOME": "/tmp/untrusted-home", "LANG": "en_US.UTF-8"})
        ...
        with mock.patch.object(amp_safe, "_account_home", return_value=account_home), \
                ...
                mock.patch.object(amp_safe.subprocess, "run", return_value=SimpleNamespace(
                    returncode=0)) as execute:
            self.assertEqual(self.run_amp("whoami"), 0)
        self.assertEqual(execute.call_count, 1)
        self.assertEqual(execute.call_args.args[0], [
            str(self.stub.resolve()), "whoami", "-o", "json", "--no-interactive"])
        child = execute.call_args.kwargs["env"]
