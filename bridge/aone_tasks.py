...
def _terraform_pure_datasource_instructions(item_id):
    ...
    return (
        ...
        "已是真人时不得改派 —— 受保护 = config/contacts.json 的 contacts 在册真人（工号非 "
        "WORKER_ 前缀且非 legacy_inbound_only）∪ product_maintainers 云产品专属维护名单"
        "（分支 A，他们不在 contacts 内）。脚本退码 3 即视为「已有真人跟进」，按原 owner "
        "继续开发、不重试、不升级；退码 0 含目标与当前一致的幂等 no-op。上面的 owner 映射"
        "只在尚无真人持单时用于首次落位，不是每轮无条件覆写。\n"
        "- 分支 A 优先：当前 assignee 已在专属维护名单内时，本轮不得重判成 pure datasource / "
        "D / G 再把单改派给过载/新山/临钧，也不发 route DM；开发、评论、PR 照常，只是 owner "
        "不动。\n"
...
def _terraform_d_g_source_only_instructions(item_id):
    ...
    return (
        ...
        "已是真人时不得改派 —— 受保护 = config/contacts.json 的 contacts 在册真人（工号非 "
        "WORKER_ 前缀且非 legacy_inbound_only）∪ product_maintainers 云产品专属维护名单"
        "（分支 A，他们不在 contacts 内）。脚本退码 3 即视为「已有真人跟进」：保留原 owner "
        "继续开发，不重试、不升级、不因此 blocked，并**跳过本轮 route DM**——DM 收件人按 "
        "subtype 固定映射，单已由真人持有时发出去会指错人。上面的 D/G owner 映射只在尚无真人"
        "持单时用于首次落位，不是每轮无条件覆写。\n"
        "- 分支 A 优先于 D/G：当前 assignee 已在专属维护名单内时，本轮不得重判成 D/G 再改派给"
        "过载/新山/临钧。route DM 的跳过是机器强制的——terraform_route_notify 会先跑 "
        "`aone-assign.sh --check`，退码 3 就返回 state=skipped_owner_protected、"
        "notification_complete=false 且不入 ledger；那是有意的 no-op，不重试、不升级，"
        "reply_body 里按「route DM 未发送（单由真人持有）」如实写。\n"
...
def _terraform_reply_template_instructions():
    ...
    return (
        ...
        "总体进展和发布安排的总结句必须加粗；待处理事项直接逐项列出，不增加重复总结句。"
        "段落及各项之间留一个空行，不把整个回复包在代码块。\n"
        "**一、完成情况**\n"
        "总体进展用一句话指出资源构建、资源测试、资源文档、Provider 发布中尚未完成的阶段及卡点；"
        "全部完成或无需变更时如实说明。按构建、测试、文档、发布分项写真实状态及已完成内容；不适用时写原因。"
        "百分比仅在有明确分子/分母时使用并注明口径，否则用状态，不估算总进度。\n"
        "**二、待处理事项及负责人**\n"
        "逐项写待办、角色及已知实名负责人、需要确认的具体决策和方案链接。"
        "发送前核实具体处理人的花名和工号，正文使用 @花名(工号)：半角括号、完整工号（保留 WB 等前缀）、不夹空格、不包反引号；"
        "多个待办分别 @ 对应人，不以团队名或裸姓名代替。未知时写「负责人待确认」及信息缺口，不编造姓名或 @；无待办明确写无。"
        "评论提交成功不代表钉钉通知已送达，不得宣称处理人已收到通知。\n"
        "**三、预计发布时间**\n"
