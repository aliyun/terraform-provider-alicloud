...
3) **工作区按登记走**：repo/路径/池/构建命令以 config/workspaces.json 为准；缺登记→escalate（missing_capability），勿臆造。**路径一律走 `bootstrap/workspace.sh dir <key>` 解析，别读 path 字段**：checkin 的 workspaces.json 不存绝对路径（每台机不同），机器本地路径放 gitignored `config/workspaces.local.json`（照 `workspaces.local.example.json` 拷，本地优先深合并）。解析序：(a) merged path 存在且目录在→用之；(b) 否则 `${JARVIS_WORKSPACE_ROOT:-~/workspace}/<repo>` 存在→用之；(c) 否则有 git_url→clone 到该处（脚本打印目标路径）再用；(d) 无 path 无 git_url→escalate（退码 4）。多数机器只需设 `JARVIS_WORKSPACE_ROOT`，无需 local 文件。
