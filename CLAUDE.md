...
1) **改文件先开 worktree，严禁直接合入主干**：任何涉及修改文件的动作，必须基于 worktree 切到新分支（或上下文已给出的分支）上进行修改与验证，禁止直接在主工作目录改文件；分支只走 PR/MR 待仓库主人人工合并，主 Agent 严禁自行 `git merge`/`push` 入 master。例外：仓库主人当面授权的指定文件可直接改 master。
