...
8) **a1 身份默认 jarvis**：跑 a1 一律走 `bin/a1id`，默认用 jarvis 身份（`a1id -- <args>`）。其余授权身份（`chenyi`=辰羿本人、`guozai`=过载本人）禁止擅用——仅在仓库主人本轮当面授权时才 `a1id as <id> -- <args>` 临时切，用完即回，绝不自持。无授权用授权身份属红线。
