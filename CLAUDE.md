...
2) **编码交子代理**：尽量用 SubAgent 处理具体编码/调试工作，主会话即编排者，只编排，保持上下文干净，不被开发细节和代码污染。本仓内置 `.claude/agents/` 三类（developer / reviewer / verifier）作首选;但主会话**不限于**这三类——可自由委派任意可用子代理（内置 general-purpose/Explore/Plan、全局 claude-code-guide、superpowers 等），按任务挑最合适的。单条工单执行由 `bootstrap/triage-one.sh` 做首尾 bookend，主会话调度它即可。不单设 jarvis 子代理（CLAUDE.md 已承担总领角色，避免重复）。
