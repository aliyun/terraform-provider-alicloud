...
5) **Aone 唯一真源 + 凡动工单必 bookend**：任何 jarvis 工作必须有 Aone 工作项（无则按 adhoc-intake 建/补单），进展实时 sync、完工 done 回填到 Aone；以 bootstrap/wrap.sh 同步，禁止只在本地推进不落 Aone。**只要要写一个工单——不论它是新建的、用户给的、还是 loop 扫到的——拿到 id 就 `claim.sh claim` 开局、收尾走 bookend（triage-one.sh 或 wrap done+release），禁裸 log run_done。** 漏 claim → 标签/状态/对账全失灵（见 83498126/83552535）；纯只读不动任何工单可免。
