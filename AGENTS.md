...
3) bridge 定时扫池，把可恢复业务统一写入控制面 Task，由 `PersistenceExecutor` lease 并发处理；probe 等一次性作业走 `EphemeralExecutor`（授权前置可配 `JARVIS_AUTO_DISPATCH=0`）。所有周期任务只由 `bridge/scheduler/jobs.py` 的七个显式定义和 `SchedulerService` 调度：`runners/aone.py` 负责 scan/claim-health/nudge，`pr.py` 负责 watch，reply/recovery/daily_probe 各自独立。扫描/派发由 bridge 全权负责，Jarvis 只被动接单；唯一入口为 `bridge/run.sh start`（自动 source env + 判定角色与钉钉/降级模式，不需额外点火）。**派发探测真源 = Aone runner 的 Python 直查并集**（每池 `assignedTo∪workitem.tracker∪tag=jarvis-idle` × `DIGITAL_WORKER_IDS`：指派/抄送数字人的新单或更新单、及 idle 人工门），非 `bootstrap/scan.sh`——后者已降级为人工审计/兜底工具与 backlog-drain 的 any-assignee 扫描。**多机部署**：`JARVIS_BRIDGE_ROLE=scheduler`（macmini 一台，DingTalk + scheduler + executor）+ `JARVIS_BRIDGE_ROLE=worker`（Linux 机器 N 台，只 executor），聚合并发 = 各机 `JARVIS_DISPATCH_MAX` 之和；详见 [docs/multi-worker-deployment.md](docs/multi-worker-deployment.md)。
