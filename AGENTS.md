...
2) **编码交子代理**：尽量用 SubAgent 处理具体编码/调试工作，主会话即编排者，只编排，保持上下文干净，不被开发细节和代码污染。本仓内置 `.Codex/agents/` 三类（developer / reviewer / verifier）作首选;但主会话**不限于**这三类——可自由委派任意可用子代理（内置 general-purpose/Explore/Plan、全局 codex-guide、superpowers 等），按任务挑最合适的。单条工单执行由 `bootstrap/triage-one.sh` 做首尾 bookend，主会话调度它即可。
...
11) **修 bug 必须补/改一个会因该 bug 失败的用例（锁定回归）**：修缺陷/bug 的 PR/CR，必须在同 PR 里补或改一个测试用例，满足「未打 patch 前会 fail、打 patch 后 pass」（unit test / acc test / integration test / repro script 皆可），随 CR 一并评审——用例与 fix 是同一次评审的两半，缺一不可。**真无法测**（概率事件无法稳定复现、需实体网关或外部依赖、上游 API 无 mock 层等）才允许豁免：在 PR/CR body 里明写「无法用例复现：<具体原因>」+ 已做的替代验证（同类资源对比 / 静态查证 / manual repro log / 生产日志佐证）；审阅人可以拒。**why**：只改代码不加测，下一版 CI 或 refactor 就会悄悄回退，客户再次踩坑——测试是回归的锚，不是可选装饰。terraform-provider 修复类走 [provider-resource-dev](.claude/skills/provider-resource-dev/SKILL.md) 步骤 6.5；其它场景同理。
