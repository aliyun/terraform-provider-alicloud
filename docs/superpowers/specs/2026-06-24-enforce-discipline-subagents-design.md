...
|------|------|------|-----------|--------|-----------|------|
| `triager` | 单条工单全流程：读单→两层查证→回复/打标/建需求/建 CR；可再调 `developer`/`verifier` | 默认 | Bash, Read, Grep, Glob, WebFetch/Search, Skill(aone-triage), aone-get/wrap/claim.sh | a1 评论/标签/状态/建单（已授权项） | 默认入口，scan/plan 出的工单都走它 | 返回 summary+status 给编排层，由 triage-one 兜底 done |
| `developer` | 改代码/调试/build/test，限 workspaces.json 登记 repo | **worktree** | 全（Edit/Write/Bash/Read/Grep） | 仅 worktree 分支，走 PR/CR，禁 master | triager 判定需落码 / 自交付路径 | build+test 绿才回，dirty 不退 |
| `pr-reviewer` | TF provider GitHub PR 评审，默认只读 | 默认 | Read, Bash(git/gh), Grep, WebFetch, Skill(terraform-pr-review) | 默认零写，授权后评论 | 出评审报告，不合并不 push | 报告回编排层 |
