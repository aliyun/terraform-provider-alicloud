...
- **Step 0 能力探测**：截图前先 `capture.sh probe`；命中通道记名进 Step 2，命中
  `missing_capability` 则把该层 manifest 写 `n-a` + 原因，继续文字查证和最终评论。
  `capture_error` 同样按原分类记录，不得误写成“无浏览器通道”。截图是证据增强项，
...
两通道都没装的 worker，manifest 逐层记录 `n-a + missing_capability`，最终聚合评论明确
“截图证据已降级”并继续业务收口，不会静默漏做，也不会卡住评论区。
