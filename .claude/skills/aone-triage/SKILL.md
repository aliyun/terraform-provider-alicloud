...
**Terraform 主处理回复**必须使用 `loops/terraform-reply-template.md`，固定四段：
**一、完成情况**、**二、待处理事项及负责人**、**三、预计发布时间**、**四、必要证据**。
“总体进展”和“发布安排”两句加粗，段落之间留空行；补料、挂起、恢复及无须开发也适用。内部查证、开发、
验收结果合并为需求方可理解的进展，技术细节放证据链接。百分比必须有明确分子分母，
发布时间必须有已确认排期；未知负责人或时间写待确认，禁止用期望日期冒充发布承诺。
总体进展只写未完成阶段及卡点；待处理事项直接逐项列明，不增加“当前待办”总结句。
每项待办核实花名与工号后使用未包裹反引号的 @花名(工号)；无法核实则写负责人待确认及缺口，
不得把评论提交成功表述为负责人已收到通知。
以下通用草稿只用于非 Terraform；后续生命周期事件继续使用既有固定通知模板。
...
- PD 必须把三层截图尝试结果和 `evidence-manifest.md` 路径放入 `visual_evidence_manifest`；finalizer 用 `screenshot-evidence/scripts/validate-manifest.py` 校验后尽力上传报告，链接只进入本轮唯一聚合回复（executor 托管时为 `AONE_RESULT.reply_body`）。缺层、缺 manifest 或上传失败必须在回复中明确说明截图降级，但不得仅因此标为 blocked/missing_capability 或挂起。
...
- ❌ 看到 bot.log 里 `scan auto: dispatched #<id> (force=True)` + `persisted Task(s)` 但无 `dispatch_item #<id> start` 就判定「Worker 没 lease / 工单没被处理」—— 实际可能 Worker 已 lease 并卡在 `PersistentTaskExecution` 的 field_repair 前置门(`field_repair_transient/apply_timeout` → SESSION_FAILED → RECOVERY_REQUIRED)，或 Task 已 `RECOVERY_REQUIRED`/retry 超限不在 READY 队列；scan 的 `force=True` 在控制面 Task 路径是**空操作**(只更新 desiredRevision，不翻转 execution status)。排查必走 `bash bootstrap/control-plane-status.sh task <id>` 看 Task `status/generation/retry/lastError/session RESUMABLE/event timeline`(看最近 `SESSION_FAILED` 与 `TASK_UPSERTED <state>→<same-state>` 空转)，详见 [docs/execution-architecture.md](../../../docs/execution-architecture.md) §「RECOVERY_REQUIRED and the desired-state upsert boundary」。案例:Task 471(aone 84103836)过载评论无人回复，真因是 field_repair `apply_timeout` → RECOVERY_REQUIRED → retry 4/3 超限卡死，scan 三次 upsert 全是 `RECOVERY_REQUIRED→RECOVERY_REQUIRED` 空转
