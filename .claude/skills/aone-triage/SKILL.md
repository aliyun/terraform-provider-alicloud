...
| **钉钉私信**(转单/关单补充通知) | `bash bootstrap/notify-dingtalk.sh <staffId> "<title>" "<body>"`;仅**转单**(承接方)与**模板 E 关单**(提单人+承接方)两类动作触发,补建/进度跟进/补料不发;缺凭据/`JARVIS_NOTIFY_DINGTALK=0`/opt-out 均静默降级不阻断。详见 `references/tf-customer-request-routing.md` §"钉钉私信 · 通用调用姿势" |
