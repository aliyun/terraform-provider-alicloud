...
2. **谜拟(479782) 关联单落 Terraform镇元对接**(2165097, `pools.upstream.cloudspec_gap`)——镇元侧根因由谜拟在自己池内跟进,tf_provider(528766) 池只放 provider 侧修复(如分支 E 紧急双单里的新山单)。**分支 E 双单时两池并存**:谜拟单 → 2165097,新山单 → 528766。
...
Aone 评论 + @花名(工号) 仍是**主通知渠道**;钉钉私信是**补充**,让承接方在 Aone 通知易被漏看时也能第一时间知道。**只在两类动作触发**:
1. **转单**——分支 A/B/C/D-新山/D-过载/D-临钧/E/G/H,给新承接方(建关联单+改 assignee 的对象)发私信
2. **关单提示**——模板 E,给**提单人 + 承接方**各发一条(提单人是阿里前线/PD,不是外部客户)

**不发**的动作:补建关联单 / 观察等待 / 模板 D 进度跟进 / 模板 F 补料提醒 —— 走 Aone 评论即可,私信只会成噪声。
...
# 4. 钉钉私信承接方(补充通知,失败不阻断;分支 E 双单时两人各发一条)
bash bootstrap/notify-dingtalk.sh <工号> \
  "Aone 转单 · <一句诉求>" \
...
|---|---|---|
| **转单** | 改**承接方**(过载/新山/谜拟/临钧/夏节/云产品专属人 —— 按分支 A-H 决策树落到谁) | 建关联单同时把主单 assignee 同步到承接方,便于承接方看到自己单子;**分支 A(专属维护名单)例外**——只改 assignee + 状态,**不建关联单**(见 Step 3 分支 A 段落) |
...
- 「缺关联单」的判定:源单看 relation list,若没有指向承接方对应池的关联单,即算缺——承接方 → 池映射:过载/新山/夏节/G/H → tf_provider(528766),谜拟 → Terraform镇元对接(2165097),临钧 → 528766(由 acube 自动建);单纯与其他客户单的 relate 不算"承接关联单"。**分支 A(专属维护名单)不适用此判定**——A 本身不建关联单,前次已将 assignee 改到名单人即为终态,不存在"缺"
