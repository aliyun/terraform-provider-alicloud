...
## 同步 provider 源码(查证前)
`bash .claude/skills/aone-triage/scripts/sync-provider.sh` —— 无库 clone,有库只 fetch 不重置(保护本地开发);repo 路径取 config/workspaces.json 的 terraform_provider.path（改这里即可换路径）。
