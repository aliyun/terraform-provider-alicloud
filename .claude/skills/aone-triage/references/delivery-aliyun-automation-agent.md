...
6. **共享文件撞兄弟 CR**:submit 报 `pipeline_submit_failed` + 代码冲突预检 → 多半是另一在飞 CR 改了同一文件(`playground.html` 高发,多需求并行)。`git merge origin/<兄弟分支>` 合入共存、解冲突、push、重 submit。先 `git log --oneline --all -- <文件>` 找兄弟分支。**永久禁止** `app pipeline exit-cr` 和等价的 `app pipeline quit`：两者都会移除最新流水线实例里的全部 CR，`bin/a1id` 与 PreToolUse 已硬拒绝。
