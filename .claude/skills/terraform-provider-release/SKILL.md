...
Ask the user for the **local working directory of the provider GitHub repo** and record it.

Every release task creates a new worktree based on that repo, aligned with `upstream/master`:

```bash
cd <provider_repo_path>
git fetch upstream
git worktree add -b feat/<aone-id>-<brief> ../<worktree_dir> upstream/master
cd ../<worktree_dir>
```
...
Run static checks only on the generated or hand-written code.

<!-- TODO: list the concrete static-check commands (gofmt / golangci-lint / etc.) -->
...
<!-- TODO: customer-issue retrospective directory and the workflow for back-filling tests from it (to be provided by the user) -->
...
> - **Internal personnel names** — developers, reviewers, PMs, or anyone surfaced from internal systems (Aone assignee, Code Review reviewers, etc.)
...
Use the `loop` skill for periodic polling, or fetch comments via `gh pr view --comments` / the gitlab-code MCP tools.
...
- **Can resolve yourself** → modify the code → push to the PR branch → reply to the comment confirming the fix
...
- [ ] Worktree is created and based on the latest `upstream/master`
