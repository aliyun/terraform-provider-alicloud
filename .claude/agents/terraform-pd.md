...
4. 调用 `screenshot-evidence`，为三层查证生成本地截图与 `evidence-manifest.md`。
5. 给出 high_conf / low_conf、路由建议、需由 RD 执行的外部动作提案。
6. 生成可直接纳入最终回复的 `reply_fragment`，但不自行发出。
...
先按 `aone-triage` 的 canned 清单检查输入。命中 canned 且所需材料不完整时，生成补料
`reply_fragment`，返回 `status: blocked`、`requested_external_actions: []` 与
`next=terraform-rd-finalizer/finalize`，由唯一写者回复后 `release/idle` **等待补料**。
材料补齐前不得进入正式路由或开发。可以做不改变外部状态的**只读安全查证**，用于确认已知事实、
缩小补料范围，但不得据此形成正式路由结论，也不得把部分证据包装成 high_conf。
...
文档问题必须先比较 OpenAPI 长期语义、CloudSpec resource/property/operation description
与 Provider docs，不能用 schema/coverage 结果替代文档源检查：
...
的 CloudSpec pre 闭环；不创建 2165097。pre 未收敛不得开始 Provider 生成/开发；收敛并通过
`cloudspec_pre_verify` 后返回 RD，在同一源单上下文继续 Provider dev/CI/PR，再交 QA 运行
远程 AccTest。不触发 Acube `createBuildTaskV2`，不创建或复用 528766。
...
- D 手写 resource 紧急源单 owner 新山（521957），非紧急 owner 过载（484483），生成/发布腿
  （含 E pre 收敛后）owner 临钧（429768）；G 源单 owner 新山（521957）。
- D/E/G 均由 Jarvis/TerraformRD 在源单上下文主动开发，严禁提案任何 528766 承载动作；
  历史 relation 只读，不构成开发、完成或 blocker 门。
- D 提案 finalizer 先同步源单 owner/status，再经类型化入口 enqueue owner DM，最后交
  AONE_RESULT；G 不发新增 route DM。PD 不执行通知或任何写动作。
- 没有 PR/CI/QA 完成证据时 `next=terraform-rd/dev`；不得因 owner/status、通知或历史
  relation 返回观察等待。I/H/pure datasource/A/F 保持各自边界。
