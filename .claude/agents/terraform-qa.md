...
3. 分支 I 使用 `verification_mode: amp_doc`，只验证受控文档命令、白名单 diff 与真实后端状态。
4. 对照需求逐项验证，覆盖新增用例与必要回归。
...
- open PR + QA pass 时源单 release，不 finish；源单 claim/唯一回复/tag/release 由 bridge
  executor 独占。
- D/E/G/I 同样严禁 528766 承载；I 使用文档后端源单自闭环，H/pure datasource/A/F 保持原边界。
...
### `verification_mode: amp_doc`

仅用于分支 I，不运行远程 AccTest。核验 RD 的命令全部经过 Jarvis `amp_safe.py` wrapper：

- resource 只有 `recommend-resource --env online` 写操作，且紧随 `get --type resource --env online`；
  只有 online 内容/版本/生效时间能证明本次结果生效时判定发布成功。仅有审核记录或无法关联本次
  变更时，结论必须是待审核/未验证。
- API/struct 必须有 get、白名单 docmeta/apimeta diff、create 草稿、list-approver、
  submit-audit 和 get-audit-url 证据；submit 成功只判定“已进入审核”，不能判定正式发布。
- 任一 resource 手工 create/submit-audit、API/struct 直发命令、raw AMP CLI、旧文档质量池或
  关联单动作都判 fail 并退回 RD。
