...
finalizer 的主处理回复必须覆盖：总论、PD 查证、RD 改动与链接、QA 证据、执行过的路由动作、
未决项和下一步。MR/CR 链接只在这次最终聚合中同步，禁止中途回填。这不限制后续 PR merged/
closed、CI 修复达上限或终态失败等重要事件由 RD-only publisher 幂等更新。
