...
- **Aone 侧唯一身份 = terraform-rd**：Terraform 对外写只有这一个身份，未登录直接报错，
  ...
  `bridge.terraform_route_notify` 幂等 enqueue；G/I 不发新增 route DM。I 严禁任何 528766 关联单；
  只有 H 的合法关联单按既有边界由 finalizer bookend；源工单由 executor 落账。后续重要事件由 bridge 的同身份
  幂等 publisher 执行。
...
- I 在源单聚合文档后端结果且严禁 528766 claim/bookend；只有 H 保持既有合法 528766
  claim/bookend。
...
输出。“总体进展”和“发布安排”两句加粗，段落之间留空行；done/idle/suspend、补料和恢复收口均使用此模板。
...
- **分支 I — CloudSpec 文档文本 metadata 源单自闭环**：resource/API/struct description、字段解释、
  NOTE 与枚举文案，且不改变字段集合、类型、约束或 CRUD。显式调用 `amp-doc-backend`，所有命令
  经 `/usr/bin/python3 -I <jarvis>/bootstrap/amp_safe.py`。resource 只执行单资源
  `recommend-resource --env online`，随后必须 `get --type resource --env online` 验证；online
  证据不能证明新内容生效时只能报待审核/未验证。API/struct 只走 get→白名单编辑→create 草稿→
  list-approver→submit-audit→get-audit-url，submit 成功绝不宣称正式发布。禁止新建、复用、关联或
  指派旧文档质量池/Provider docs 兜底单；QA 使用 `verification_mode: amp_doc` 复核结果语义。
...
- D/E/G/I 同样严禁 528766 承载；只有 H 保持既有合法 528766 边界，pure
  datasource/A/F 保持原边界。
...
- build/test/CI/QA fail 留在 RD ↔ QA 修复闭环。open PR + QA pass 时源单 release 不 finish；
  正式发布仍为人工硬门。I 的源单自闭环严禁 528766 承载，只有 H 的合法 528766 与 pure datasource/A/F
  反向保护保持不变。
