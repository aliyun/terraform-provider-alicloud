...
#
# Owner-scoped block:
#   self is the current stable interactive session id. Each open/touched id's owner is looked up from the
#   claims ledger (id→owner map, first non-empty owner wins).
#     · owner non-empty AND != self → NOT counted as block; a WARN line is
#       printed for visibility. A session must
...
#   A foreign session's claim is skipped regardless of liveness; control-plane
#   lease expiry and the reaper own recovery of abandoned sessions.
#
# Exit codes:
...
# self via claim_owner() (lib.sh): explicit JARVIS_CLAIM_OWNER,
# cc-<CLAUDE_CODE_SESSION_ID>, codex-<CODEX_THREAD_ID>, or "".
...
# No claim/touched ledgers → skip the run_done sweep.
...
if [ "${#open_ids[@]}" -gt 0 ]; then
    ...
    for id in "${open_ids[@]}"; do
        ...
        if [ -n "$owner" ] && [ "$owner" != "$self" ]; then
            ...
            # visibility; the control-plane reaper owns abandoned-session recovery.
