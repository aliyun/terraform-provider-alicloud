#!/usr/bin/env bash
set -uo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"; R="$(jarvis_root)"
I="$R/.my-day/instances"; T="$R/.my-day/tasks"; TTL="${COORD_TTL:-180}"
mkdir -p "$I" "$T"; cmd="${1:-}"
case "$cmd" in
 register) role="${2:-triage}"; pid="${3:-$$}"; id="$(hostname)-$pid"; umask 077
   # pid is the caller's LONG-LIVED pid (pass "$$" from your script). Without it we
   # fall back to $$ = coord.sh's own short-lived subshell pid, which exits the moment
   # register returns — making `dead` (kill -0 on the embedded pid) report the instance
   # dead immediately. Callers that want real liveness MUST pass their own $$.
   printf '{"id":"%s","role":"%s","pid":%s,"host":"%s","started":"%s","task":null}' \
     "$id" "$role" "$pid" "$(hostname)" "$(date -u +%FT%TZ)" > "$I/$id.json"; : > "$I/$id.hb"; echo "$id";;
 heartbeat) umask 077; : > "$I/${2}.hb";;
 dead) f="$I/${2}.hb"; pid="${2##*-}"; [ -f "$f" ] || exit 0
   m=$(stat -c %Y "$f" 2>/dev/null||stat -f %m "$f" 2>/dev/null); age=$(( $(date +%s)-m ))
   [ "$age" -gt "$TTL" ] && exit 0
   kill -0 "$pid" 2>/dev/null && exit 1 || exit 0;;
 checkpoint) aid="$2"; st="$3"; wt="${4:-}"; br="${5:-}"; rp="${6:-}"; pb="${7:-}"; umask 077; tmp=$(mktemp "$T/.t.XXXX"); printf '{"aone_id":"%s","owner_instance":"%s","stage":"%s","worktree":"%s","branch":"%s","repo":"%s","pushed_branch":"%s","updated":"%s"}' "$aid" "${COORD_ID:-}" "$st" "$wt" "$br" "$rp" "$pb" "$(date -u +%FT%TZ)" >"$tmp" && mv "$tmp" "$T/$aid.json";;
 list-orphans) for f in "$T"/*.json; do [ -e "$f" ]||continue; st=$(jq -r '.stage // ""' "$f"); [ "$st" = "done" ] && continue; o=$(jq -r .owner_instance "$f"); [ -n "$o" ]&&[ "$o" != null ] && bash "$0" dead "$o" && jq -r .aone_id "$f"; done;;
 adopt) f="$T/$2.json"; [ -f "$f" ]||exit 1; tmp=$(mktemp "$T/.a.XXXX"); jq --arg i "${COORD_ID:-}" '.owner_instance=$i' "$f">"$tmp"&&mv "$tmp" "$f";;
 *) echo "usage: coord.sh {register|heartbeat|dead|checkpoint|list-orphans|adopt}" >&2; exit 2;; esac
