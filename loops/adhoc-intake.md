...
5. **碰已有工单必开 bookend**：复用现有工单(triage/回复/改标/改状态)的，**开局即 `bootstrap/claim.sh claim <id> <project>`**(打 jarvis-claimed、入台账)，收尾走 `wrap.sh done`(内含 release)。漏 claim = 后续标签/状态/对账全失灵(see 83498126)。纯本地只读任务可跳过本节直接进四。
...
> **收尾必走 bookend，禁裸 `log.sh run_done`**：碰已有工单的，收尾用 `bootstrap/triage-one.sh <id> <pool> <project> "<summary>" <status>`(claim→done→release 一把成对)，或手跑 `wrap.sh done` + `claim.sh release` 两步。裸 run_done 只落本地审计：标签停 claimed/状态不动/release 漏。claim 开头、release 收尾闭合，`wrap-check.sh`+`reconcile.sh` 才兜得住(see 83498126)。
