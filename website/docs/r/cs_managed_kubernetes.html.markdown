...
* `is_enterprise_security_group` - (Optional, Available in 1.91.0+) Enable to create advanced security group. Default to `false`. See [Advanced security group](https://www.alibabacloud.com/help/doc-detail/120621.htm).
* `proxy_mode` - (Optional) Proxy mode is option of kube-proxy. options: `iptables`|`ipvs`. Default to `ipvs`.
...
* `tags` - (Optional, Available in 1.97.0+) Default to nil, A map of tags assigned to the kubernetes cluster and work nodes . Detailed below.
...
* `load_balancer_spec` - (ForceNew, Available in 1.117.0+) The cluster api server load balance instance specification. Default to `slb.s1.small`. For more information on how to select a LB instance specification, see [SLB instance overview](https://help.aliyun.com/document_detail/85931.html).
* `control_plane_log_ttl` - (Optional, ForceNew, Available in 1.141.0+) Control plane log retention duration (unit: day). Default to `30`. If control plane logs are to be collected, `control_plane_log_ttl` and `control_plane_log_components` must be specified.
...
* `weekly_period` - (Required) Maintenance cycle, you can set the values from Monday to Sunday, separated by commas when the values are multiple. Default to `Thursday`.
...
* `node_cidr_mask` - (Optional) The node cidr block to specific how many pods can run on single node. `24-28` is allowed. 24 means 2^(32-24)-1=255 and the node can run at most 255 pods. Default to `24`.
...
* `password` - (Optional, Sensitive) The password of ssh login cluster node. You have to specify one of `password` `key_name` `kms_encrypted_password` fields. From version 1.109.1, it is not necessary in the professional managed cluster.
* `key_name` - (Optional) The keypair of ssh login cluster node, you have to create it first. You have to specify one of `password` `key_name` `kms_encrypted_password` fields. From version 1.109.1, it is not necessary in the professional managed cluster.
...
* `worker_instance_charge_type` - (Optional, ForceNew) Worker payment type, its valid value is either or `PostPaid` or `PrePaid`. Default to `PostPaid`. If value is `PrePaid`, the files `worker_period`, `worker_period_unit`, `worker_auto_renew` and `worker_auto_renew_period` are required.
...
* `worker_auto_renew` - (Optional) Enable worker payment auto-renew. Default to `false`.
...
* * `slb_id` - (Available in 1.173.0+) The API Server load balancer id.
