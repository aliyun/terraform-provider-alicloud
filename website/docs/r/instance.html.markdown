...
* `managed_host_id` - (Optional, ForceNew, Available since v1.289.0) The ID of the managed host on which to create the instance, such as `mh-f2d3647ca21****`. The managed host refers to the physical machine in the intelligent fully-managed resource pool.

  -> **NOTE:** The managed host of an instance cannot be changed after creation. The ECS APIs do not return the managed host ID when describing instances, so Terraform cannot read this attribute back. After importing an existing instance, this field shows an empty value; modifying it in the configuration triggers a replacement of the instance.
