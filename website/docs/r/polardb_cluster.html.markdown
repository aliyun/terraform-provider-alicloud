...
* `creation_option` - (Optional, Available in 1.153.0+) The method that is used to create a cluster. Valid values:
...
* `creation_category` - (Optional, Available in 1.153.0+) The edition of the PolarDB service. Valid values:
...
* `source_resource_id` - (Optional, Available in 1.153.0+) The ID of the source RDS instance or the ID of the source PolarDB cluster. This parameter is required only when CreationOption is set to MigrationFromRDS, CloneFromRDS, or CloneFromPolarDB.
...
* `gdn_id` - (Optional, Available in 1.153.0+) The ID of the global database network (GDN).
  -> **NOTE:** This parameter is required if CreationOption is set to CreateGdnStandby.
* `clone_data_point` - (Optional, Available in 1.153.0+) The time point of data to be cloned. Valid values:
