...
* `name` - (Optional, Available in 1.44.0+) Field `name` has been deprecated from provider version 1.120.0. New field `forward_entry_name` instead.
* `forward_entry_name` - (Optional, Available in 1.120.0+) The name of forward entry.
...
* `port_break` - (Optional, Available in 1.120.0+) Specifies whether to remove limits on the port range. Default value is `false`.
...
* `status` - (Available in 1.120.0+) The status of forward entry.
