...
* `scaling_rule_type` - (Optional, Available in 1.57.2+) The scaling rule type, either "SimpleScalingRule", "TargetTrackingScalingRule", "StepScalingRule". Default to "SimpleScalingRule".
* `estimated_instance_warmup` - (Optional, Available in 1.57.2+) The estimated time, in seconds, until a newly launched instance will contribute CloudMonitor metrics. Default to 300.
* `metric_name` - (Optional, Available in 1.57.2+) A CloudMonitor metric name.
* `target_value` - (Optional, Available in 1.57.2+) The target value for the metric.
* `disable_scale_in` - (Optional, Available in 1.57.2+) Indicates whether scale in by the target tracking policy is disabled. Default to false.
* `step_adjustment` - (Optional, Available in 1.57.2+) Steps for StepScalingRule. See [Block stepAdjustment](#block-stepAdjustment) below for details.
