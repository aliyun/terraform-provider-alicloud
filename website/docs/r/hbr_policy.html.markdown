...
* `immutable` - (Optional, Computed, Bool) Specifies whether to enable the backup lock (immutable backup) for the rule. Valid only when `PolicyType` is set to `UDM_ECS_ONLY`. When enabled, backups generated by this rule cannot be deleted until the retention period expires.
...
* `rule_type` - (Required) Rule Type. Valid values: `BACKUP`, `TRANSITION`, `REPLICATION`, `TAG` and `SECURITY`. The `SECURITY` value is used for immutable backup rules, where backups cannot be deleted until the retention period expires.
