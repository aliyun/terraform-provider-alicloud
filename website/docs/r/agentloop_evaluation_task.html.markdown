...
resource "alicloud_agentloop_evaluation_task" "default" {
  ...
  evaluators {
    ...
    type          = alicloud_agentloop_evaluator.default.type
...
* `data_filter` - (Optional) The filter expression of the data to be evaluated, as a JSON object string (e.g. `jsonencode({ query = "checkout-service", maxRecords = 10 })`). Formatting differences such as key order and whitespace are suppressed.
