...
* `resource_tag` - (Optional, ForceNew) The tags used to filter target ECS instances for batch command execution. Each tag contains a `key` and `value`. **NOTE:** `resource_tag` conflicts with `instance_id`. When `resource_tag` is specified, `instance_id` cannot be set. See [`resource_tag`](#resource_tag) below.
...
The tags used to filter target ECS instances for batch command execution. Each tag contains a `key` and `value`. **NOTE:** `resource_tag` conflicts with `instance_id`. When `resource_tag` is specified, `instance_id` cannot be set.
