...
subcategory: "RDS"
layout: "alicloud"
page_title: "Alicloud: alicloud_db_clone_instance"
sidebar_current: "docs-alicloud-resource-db-clone-instance"
description: |-
  Provides an RDS instance resource.
---

# alicloud\_db\_clone\_instance
...
```terraform
resource "alicloud_db_clone_instance" "example" {
  db_instance_id       = "your primary db_instance_id"
  db_instance_storage_type       = "cloud_ssd"
  instance_charge_type        = "Postpaid"
  restore_time     = "2021-10-20T16:00:00Z"
}
```
...
* `db_instance_storage_type` - (Required) The storage type of the new instance. Valid values:
...
* `zone_id` - (Optional) The ID of the zone to which the new instance belongs. You can call the [DescribeRegions](https://www.alibabacloud.com/help/doc-detail/26243.htm) operation to query the most recent zone list.

-> **NOTE:**
- The default value is the ID of the zone to which the original instance belongs.
* `instance_type` - (Optional) The instance type of the new instance. For more information, see [Primary instance types](https://www.alibabacloud.com/help/doc-detail/26312.htm).

-> **NOTE:**
- The default value is the instance type of the original instance.
* `instance_storage` - (Optional) The storage capacity of the new instance. Unit: GB. The storage capacity increases in increments of 5 GB. For more information, see [Primary instance types](https://www.alibabacloud.com/help/doc-detail/26312.htm).

-> **NOTE:**
- The default value is the storage capacity of the original instance.
* `db_names` - (Optional) The names of the databases that you want to create on the new instance.
* `backup_id` - (Optional) The ID of the backup set that you want to use.You can call the [DescribeBackups](https://www.alibabacloud.com/help/doc-detail/26273.htm) operation to query the most recent backup set list.
...
-> **NOTE:**
- You must specify at least one of the `BackupId` and `RestoreTime` parameters.
* `restore_table` - (Optional) Specifies whether to restore specific databases and tables. The value 1 specifies to restore specific databases and tables. If you do not want to restore specific databases or tables, you can choose not to specify this parameter.
...
* `dedicated_host_group_id` - (Optional) The ID of the dedicated cluster to which the new instance belongs if you create the new instance in a dedicated cluster.  
* `backup_type` - (Optional) The type of backup used by the new instance. Valid values:
...
```
$ terraform import alicloud_db_clone_instance.example rm-abc12345678
```
