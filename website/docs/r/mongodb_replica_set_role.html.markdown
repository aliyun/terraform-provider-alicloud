...
For information about how to modify connection string of MongoDB, see [Modify Connection String](alibabacloud.com/help/en/mongodb/user-guide/change-the-endpoint-and-port-of-an-instance).
...
resource "alicloud_mongodb_replica_set_role" "public" {
  count = length(alicloud_mongodb_public_network_address.default.replica_sets)
  db_instance_id = alicloud_mongodb_instance.default.id
  role_id = alicloud_mongodb_public_network_address.default.replica_sets[count.index].role_id
  connection_prefix = "test-tf-public-${count.index}"
...
* `network_type` - (Required) The network type of the connection string. Valid values:
...
* `connection_prefix` - The prefix of the connection string.
* `connection_port` - The port of the connection string.
