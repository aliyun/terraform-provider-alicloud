...
* `sql_collector_status` - (Optional, Available in 1.69.0+) The sql collector status of the instance. Valid values are `Enable`, `Disable`, Default to `Disable`.
* `sql_collector_config_value` - (Optional, Available in 1.69.0+) The sql collector keep time of the instance. Valid values are `1`, `30`, `180`, `365`, `1095`, `1825`, `1` is the initial value, and can't change it to `1`.
