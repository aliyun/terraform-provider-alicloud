...
* `pg_bouncer_enabled`- (Optional, Available since 1.238.0) Modify the PgBouncer feature of the RDS PostgreSQL instance. Valid values:
  - true: enable.
  - false: disable.
* `recovery_model` - (Optional, Available since 1.238.0) Enable the Simple Recovery Model for an RDS SQL Server Instance.The Simple Recovery Model feature is only supported by the Basic Series of RDS SQL Server instances. Once this feature is enabled, it cannot be disabled.Valid values:
