...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_ecs_activation&spm=docs.r.ecs_activation.example&intl_lang=EN_US)
...
* `resource_group_id` - (Optional, ForceNew, Computed) The ID of the resource group to which the activation code belongs.
* `disabled` - (Optional, Computed) Specifies whether to disable the activation code. Valid values: `true`, `false`. Default value: `false`.

-> **NOTE:** Once the activation code is disabled (`disabled = true`), it cannot be enabled again because the EnableActivation API does not exist. Disabling is a one-way operation.
...
* `activation_code` - (Sensitive) The code of the activation code. This code is returned only once when calling the CreateActivation operation and cannot be queried later. Please save it properly.
