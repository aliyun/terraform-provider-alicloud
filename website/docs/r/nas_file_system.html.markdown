...
description: |-
  Provides a Alicloud Network Attached Storage (NAS) File System resource.
---

# alicloud_nas_file_system

Provides a Network Attached Storage (NAS) File System resource.

File System Instance.

For information about Network Attached Storage (NAS) File System and how to use it, see [What is File System](https://www.alibabacloud.com/help/en/nas/developer-reference/api-nas-2017-06-26-createfilesystem).
...
* `keytab` - (Optional, Available since v1.247.0) String of keytab file content encrypted by base64
* `keytab_md5` - (Optional, Available since v1.247.0) String of the keytab file content encrypted by MD5
...
* `options` - (Optional, List, Available since v1.247.0) Option. See [`options`](#options) below.
...
* `smb_acl` - (Optional, List, Available since v1.247.0) SMB ACL See [`smb_acl`](#smb_acl) below.
...
* `enable_oplock` - (Optional, Available since v1.247.0) Whether to enable the OpLock function. Value:
  - true: On.
...
* `enable_anonymous_access` - (Optional, Available since v1.247.0) Whether to allow anonymous access.
  - true: Allow anonymous access.
  - false (default): Anonymous access is not allowed.
* `enabled` - (Optional, Available since v1.247.0) Whether SMB ACL is enabled
* `encrypt_data` - (Optional, Available since v1.247.0) Whether transmission encryption is enabled.
  - true: Enables encryption in transit.
  - false (default): Transport encryption is not enabled.
* `home_dir_path` - (Optional, Available since v1.247.0) The user directory home path for each user. The file path format is as follows:
...
* `reject_unencrypted_access` - (Optional, Available since v1.247.0) 
...
* `super_admin_sid` - (Optional, Available since v1.247.0) The ID of the Super User. The ID rules are as follows:
...
Network Attached Storage (NAS) File System can be imported using the id, e.g.
