...
* `cluster_backup_id` - (Optional, Available since v1.223.1) The ID of the backup set of the cluster.  .
...
* `node_type` - (Optional, Available since v1.223.1) Node type, value:  MASTER_SLAVE: high availability (dual copy) STAND_ALONE: single copy double: double copy single: single copy Note For Cloud Native instances, select MASTER_SLAVE or STAND_ALONE. For Classic instances, select double or single.
...
* `slave_read_only_count` - (Optional, Available since v1.223.1) Specifies the number of read-only nodes in the secondary zone when creating a multi-zone read/write splitting instance. Note: To create a multi-zone read/write splitting instance, slaveadonlycount and SecondaryZoneId must be specified at the same time.
