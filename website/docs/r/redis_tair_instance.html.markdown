...
* `param_no_loose_sentinel_enabled` - (Optional, Available since v1.233.1) sentinel compatibility mode, applicable to non-cluster instances. For more information about parameters, see yes or no in the https://www.alibabacloud.com/help/en/redis/user-guide/use-the-sentinel-compatible-mode-to-connect-to-an-apsaradb-for-redis-instance, 取值为. The default value is no.
* `param_repl_mode` - (Optional, Available since v1.233.1) The value is semisync or async. The default value is async.
...
* `security_ip_group_name` - (Optional, Available since v1.233.1) Security IP group name
* `security_ips` - (Optional, Available since v1.233.1) Security IPs
...
* `storage_size_gb` - (Optional, ForceNew, Computed, Int) The value range of different specifications is different, see [ESSD-based instances](https://www.alibabacloud.com/help/en/tair/product-overview/essd-based-instances). When the value of instance_type is "tair_essd", this attribute takes effect and is required.
...
* `vpc_auth_mode` - (Optional, Available since v1.233.1) The VPC authentication mode. Valid values: Open (enables password authentication), Close (disables password authentication and enables [password-free access](https://www.alibabacloud.com/help/en/apsaradb-for-redis/latest/enable-password-free-access)).
