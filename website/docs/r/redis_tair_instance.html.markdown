...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_redis_tair_instance&spm=docs.r.redis_tair_instance.example&intl_lang=EN_US)

## Argument Reference

The following arguments are supported:
* `additional_bandwidth` - (Optional) The additional bandwidth to add for the instance. Unit: MB/s. The maximum value is 6 times the default bandwidth of the instance (or single shard) specification, capped at 192 MB/s. When `additional_bandwidth_node_id` specifies multiple shard IDs, pass comma-separated bandwidth values in the same order. **NOTE:** This attribute is write-only and cannot be read back; Terraform keeps the configured value in the state. The additional bandwidth is charged on a pay-as-you-go basis.
* `additional_bandwidth_node_id` - (Optional) The IDs of the data shards whose bandwidth is adjusted. You can call `DescribeLogicInstanceTopology` to obtain them. Separate multiple IDs with commas (,), or set to `All` for all data shards. Required and available only for cluster or read/write splitting architectures. **NOTE:** This attribute is write-only and cannot be read back; Terraform keeps the configured value in the state.
* `bandwidth_burst` - (Optional, Bool) Specifies whether to enable Elastic Burst for the instance. After it is enabled, the instance can temporarily burst beyond the default bandwidth limit during bandwidth auto scaling. **NOTE:** This attribute is write-only and cannot be read back; Terraform keeps the configured value in the state.
...
* `maintain_end_time` - (Optional, Computed, Available since v1.286.0) The end time of the maintenance window of the Tair instance. The time is in the `HH:mmZ` format (UTC). The interval between the start time and the end time must be at least 1 hour. Example: `06:00Z`.
* `maintain_start_time` - (Optional, Computed, Available since v1.286.0) The start time of the maintenance window of the Tair instance. The time is in the `HH:mmZ` format (UTC). Example: `02:00Z`.
...
* `security_group_id` - (Optional) Security group id. Multiple security group IDs can be bound at once by separating them with a comma (`,`); the order of the IDs is not significant and does not trigger a diff.
