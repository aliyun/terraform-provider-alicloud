...
* `status` - The status returned by the API, for example `DO_CREATE` or `RUN`. `RUN` does not guarantee that all instance metadata is available yet.
...
* `create` - (Defaults to 10 mins) Used when creating the DRDS instance and waiting for `RUN`, then for its instance attributes to become available.
* `update` - (Defaults to 5 mins) Used when updating the DRDS instance.
