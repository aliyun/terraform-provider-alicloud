...
* `deletion_protection` - (Optional, Available since v1.234.1) The deletion protection of EMR cluster.
...
* `vswitch_ids` - (Optional) Global vSwitch ids, you can also specify it in node group. **NOTE:** From version 1.234.1, `vswitch_ids` can be modified.
* `with_public_ip` - (Optional) Whether the node has a public IP address enabled. **NOTE:** From version 1.234.1, `with_public_ip` can be modified.
* `additional_security_group_ids` - (Optional) Additional security Group IDS for Cluster, you can also specify this key for each node group. **NOTE:** From version 1.234.1, `additional_security_group_ids` can be modified.
* `instance_types` - (Required) Host Ecs instance types. **NOTE:** From version 1.234.1, `instance_types` can be modified.
...
* `spot_strategy` - (Optional, Available since v1.234.1) The spot strategy configuration of emr cluster.
* `cost_optimized_config` - (Optional) The detail cost optimized configuration of emr cluster. See [`cost_optimized_config`](#node_groups-cost_optimized_config) below. **NOTE:** From version 1.234.1, `cost_optimized_config` can be modified.
* `deployment_set_strategy` - (Optional, Available since v1.219.0) Deployment set strategy for this cluster node group. Supported value: NONE, CLUSTER or NODE_GROUP. **NOTE:** From version 1.234.1, `deployment_set_strategy` can be modified.
...
* `ack_config` - (Optional, Available since v1.234.1) The node group of ack configuration for emr cluster to deploying on kubernetes. See [`ack_config`](#node_groups-ack_config) below.
