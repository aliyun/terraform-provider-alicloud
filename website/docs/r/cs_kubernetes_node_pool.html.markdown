...
resource "alicloud_cs_kubernetes_node_pool" "private_pool_first" {
  node_pool_name       = "${var.name}-first"
  cluster_id           = data.alicloud_cs_managed_kubernetes_clusters.default.ids.0
  vswitch_ids          = [data.alicloud_vswitches.vsw1.ids.0]
  instance_charge_type = "PostPaid"
  system_disk_category = "cloud_essd"
  system_disk_size     = 120
  multi_az_policy      = "PRIORITY"
  desired_size         = 0
  instance_types       = [data.alicloud_instance_types.cloud_essd.instance_types.0.id]

  resource_pool_options {
    strategy         = "PrivatePoolFirst"
    private_pool_ids = [alicloud_ecs_capacity_reservation.default.id]
  }
}

# PrivatePoolOnly: use the specified private pools only; private_pool_ids is required.
resource "alicloud_cs_kubernetes_node_pool" "private_pool_only" {
  node_pool_name       = "${var.name}-only"
  cluster_id           = data.alicloud_cs_managed_kubernetes_clusters.default.ids.0
  vswitch_ids          = [data.alicloud_vswitches.vsw1.ids.0]
  instance_charge_type = "PostPaid"
  system_disk_category = "cloud_essd"
  ...
  resource_pool_options {
    ...
    private_pool_ids = [
      ...
      alicloud_ecs_elasticity_assurance.default.elasticity_assurance_id,
    ]
  }
}
```
...
resource "alicloud_cs_kubernetes_node_pool" "upgrade_nodepool" {
  ...
  upgrade_policy {
    ...
    kubernetes_version = alicloud_cs_managed_kubernetes.default.version
  ...
  rolling_policy {
    ...
    # Specific node names to be upgraded 
    # node_names = []
  }
}
```
...
* `resource_pool_options` - (Optional, List, Available since v1.292.0) The resource pool and resource pool strategy used when launching instances. See [`resource_pool_options`](#resource_pool_options) below.
...
* `upgrade_policy` - (Optional, Set, Available since v1.269.0) Configuration block for node pool upgrade operations. This is a transient parameter that triggers node pool upgrades when specified. Once the upgrade completes, this block should be removed from your configuration to prevent unintended re-upgrades on subsequent applies. See [`upgrade_policy`](#upgrade_policy) below.
...
-> **NOTE:** The API uses full-replacement semantics for `os_config`: removing the whole `os_config` block clears all custom OS configuration on the cloud side, and an empty `os_config {}` block applies no parameters (any previously applied values are cleared) while remaining in the state as an empty block. Removing a key from the `sysctl` map or from the `hugepage` block removes the corresponding parameter from the cloud-side configuration.
...
The resource pool and resource pool strategy used when launching instances. A resource pool consists of the private pools generated after an elasticity assurance service or a capacity reservation service takes effect, and the public pool, for instances to select from when they start.

-> **NOTE:** `resource_pool_options` only takes effect when creating PostPaid (pay-as-you-go) instances, and it cannot be set together with `private_pool_options.private_pool_options_match_criteria` and `private_pool_options.private_pool_options_id`. To disable the resource pool strategy, set `strategy = "None"`. This is the only supported way to disable it; removing the whole `resource_pool_options` block is not supported and may cause a permanent diff.

The resource_pool_options supports the following:
* `strategy` - (Optional) The resource pool strategy used when launching instances. Default value: `None`. Valid values:
...
* `private_pool_ids` - (Optional, List) The list of private pool IDs, that is, the IDs of elasticity assurance services or capacity reservation services. Only Target mode private pool IDs can be passed in. The value of N ranges from 1 to 20. For example, `eap-bp67acfmxazb4****`.

### `rolling_policy`

The rolling_policy supports the following:
* `batch_interval` - (Optional, Int, Available since v1.269.0) The upgrade interval time between batches, in minutes. This parameter only takes effect when `pause_policy` is set to `NotPause`.
* `max_parallelism` - (Optional, Int) The maximum number of nodes that can be upgraded in parallel per batch when updating nodes in the node pool.
* `node_names` - (Optional, List, Available since v1.269.0) Specify the list of nodes to be upgraded.
* `pause_policy` - (Optional, Available since v1.269.0) The auto-pause policy during node upgrade. Valid values:
