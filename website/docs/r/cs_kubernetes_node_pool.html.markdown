...
* `cis_enabled` - (Optional, ForceNew, Deprecated since v1.224.0) Whether enable worker node to support cis security reinforcement, its valid value `true` or `false`. Default to `false` and apply to AliyunLinux series. See [CIS Reinforcement](https://help.aliyun.com/document_detail/223744.html).
...
* `kubelet_configuration` - (Optional) Kubelet configuration parameters for worker nodes. See [`kubelet_configuration`](#kubelet_configuration) below. More information in [Kubelet Configuration](https://kubernetes.io/docs/reference/config-api/kubelet-config.v1beta1/). See [`kubelet_configuration`](#kubelet_configuration) below.
...
* `period` - (Optional) Node payment period. Its valid value is one of {1, 2, 3, 6, 12}.
...
* `security_hardening_os` - (Optional, ForceNew) Alibaba Cloud OS security reinforcement. Value:
...
* `system_disk_provisioned_iops` - (Optional) The predefined IOPS of a system disk. Valid values: 0 to min{50,000, 1,000 × Capacity - Baseline IOPS}. Baseline IOPS = min{1,800 + 50 × Capacity, 50,000}. This parameter is supported only when `system_disk_category` is set to `cloud_auto`.
...
* `max_unavailable` - (Optional) Maximum number of unavailable nodes. Default value: 1. Value range:\[1,1000\].
* `surge` - (Optional, Deprecated since v1.219.0) Number of additional nodes. You have to specify one of surge, surge_percentage.
* `surge_percentage` - (Optional, Deprecated since v1.219.0) Proportion of additional nodes. You have to specify one of surge, surge_percentage.
...
* `eip_internet_charge_type` - (Optional) EIP billing type. `PayByBandwidth`: Charged at fixed bandwidth. `PayByTraffic`: Billed as used traffic. It works if `is_bond_eip=true`. Default: `PayByBandwidth`. Conflict with `internet_charge_type`, EIP and public network IP can only choose one.
