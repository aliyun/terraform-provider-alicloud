...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_ecd_ad_connector_office_site&spm=docs.r.ecd_ad_connector_office_site.example&intl_lang=EN_US)

## Argument Reference

The following arguments are supported:

* `ad_connector_office_site_name` - (Required, ForceNew) The name of the workspace. The name must be 2 to 255 characters in length. It must start with a letter and cannot start with `http://` or `https://`. It can contain digits, colons (:), underscores (_), and hyphens (-).
...
* `dns_address` - (Required, ForceNew) The IP address N of the DNS server of the enterprise AD system. You can specify only one IP address.
* `domain_name` - (Required, ForceNew) The domain name of the enterprise AD system. You can register each domain name only once.
* `domain_password` - (Optional) The password of the domain administrator. The password can be up to 64 characters in length.
* `domain_user_name` - (Optional, ForceNew) The username of the domain administrator. The username can be up to 64 characters in length.
...
* `mfa_enabled` - (Optional, ForceNew) Specifies whether to enable multi-factor authentication (MFA).
...
* `sub_domain_dns_address` - (Optional, ForceNew) The DNS address N of the enterprise AD subdomain. If you specify a value for the `sub_domain_name` parameter but you do not specify a value for this parameter, the DNS address of the subdomain is the same as the DNS address of the parent domain.
* `sub_domain_name` - (Optional, ForceNew) The domain name of the enterprise AD subdomain.
