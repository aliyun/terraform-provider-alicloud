...
resource "alicloud_esa_rate_plan_instance" "resource_RatePlanInstance_routineRelatedRecord_example" {
  type         = "NS"
  auto_renew   = false
  period       = "1"
  ...
  coverage     = "overseas"
  auto_pay     = true
  plan_name    = "rongbeiexample2"
}

resource "alicloud_esa_routine" "resource_Routine_Record_example" {
  description = "example-routine2"
  name        = "example-routine2"
}

resource "alicloud_esa_site" "resource_Site_routineRelatedRecord_example" {
  site_name   = "chenxin0116.site"
  instance_id = alicloud_esa_rate_plan_instance.resource_RatePlanInstance_routineRelatedRecord_example.id
  coverage    = "overseas"
  access_type = "NS"
}
...
resource "alicloud_esa_routine_related_record" "default" {
  ...
  site_id     = alicloud_esa_site.resource_Site_routineRelatedRecord_example.id
  name        = alicloud_esa_routine.resource_Routine_Record_example.id
}
```
