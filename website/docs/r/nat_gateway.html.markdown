...
* `network_type` - (Optional, Computed, Available in 1.136.0+) Indicates the type of the created NAT gateway. Valid values `internet` and `internet`. `internet`: Internet NAT Gateway. `internet`: VPC NAT Gateway.
