...
* `role_arn` - (Required) The ARN of the RAM role that the digital employee assumes. The RAM role's trust policy must grant `sts:AssumeRole` to the Cloud Monitor service principal `cloudmonitor.aliyuncs.com`, as shown in the example above.
...
* `resource_group_id` - (Optional, ForceNew) The ID of the resource group to which the digital employee belongs. It can only be set at creation time.
...
* `tags` - (Optional, ForceNew) A set of tags for the digital employee. Tags can only be set at creation time; changing them forces a new resource. See [`tags`](#tags) below.
...
* `decision` - (Optional) The execution policy after the API is matched. Valid values: `deny`, `user_ack`, `allow`.
...
* `key` - (Optional, ForceNew) The key of the tag.
* `value` - (Optional, ForceNew) The value of the tag.
