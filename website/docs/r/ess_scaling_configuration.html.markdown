...
* `security_enhancement_strategy` - (Optional, ForceNew, Available since v1.231.0) Specifies whether to enable Security Hardening. Valid values: Active, Deactive.
* `instance_description` - (Optional, Available since v1.231.0) The description of ECS instances. The description must be 2 to 256 characters in length. It can contain letters but cannot start with http:// or https://.
* `spot_duration` - (Optional, Available since v1.231.0) The protection period of preemptible instances. Unit: hours. Valid values: 1, 0.
...
* `system_disk_kms_key_id` - (Optional, Available since v1.231.0) The ID of the KMS key that you want to use to encrypt the system disk.
* `system_disk_encrypt_algorithm` - (Optional, Available since v1.231.0) The algorithm that you want to use to encrypt the system disk. Valid values: AES-256, SM4-128.
* `system_disk_provisioned_iops` - (Optional, Available since v1.231.0) IOPS measures the number of read and write operations that an EBS device can process per second. 
* `image_options_login_as_non_root` - (Optional, Available since v1.231.0) Specifies whether to use ecs-user to log on to an ECS instance. For more information, see Manage the username used to log on to an ECS instance. Valid values: true, false. Default value: false.
* `deletion_protection` - (Optional, Available since v1.231.0) Specifies whether to enable the Release Protection feature for ECS instances. This parameter is applicable to only pay-as-you-go instances. You can use the parameter to specify whether an ECS instance can be directly released by using the ECS console or calling the DeleteInstance operation. Valid values: true, false. Default value: false.
...
* `provisioned_iops` - (Optional, Available since v1.231.0) IOPS measures the number of read and write operations that an Elastic Block Storage (EBS) device can process per second.
