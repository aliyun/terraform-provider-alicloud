...
* `DbAudit`- (Optional, Available in 1.127.0) Audit Log Switch.
* `Retention`- (Optional, Available in 1.127.0) Audit log Retention days.
* `availability_zone` - (Optional) It has been removed from provider version 1.127.0 and use `zone_id` instead.
* `instance_name` - (Optional) It has been removed from provider version 1.127.0 and use `db_instance_name` instead.
* `instance_charge_type` - (Optional) It has been removed from provider version 1.127.0 and use `payment_type` instead.
* `parameters` - (Optional) It has been removed from provider version 1.127.0 and use `config` instead.
* `enable_public` - (Optional, Available in v1.94.0+) It has been removed from provider version 1.127.0 and use resource `alicloud_kvstore_connection` instead.
* `connection_string_prefix` - (Optional, Available in v1.94.0+) It has been removed from provider version 1.127.0 and resource use `alicloud_kvstore_connection` instead.
* `node_type`- (Optional, Available in 1.101.0+) "Field 'node_type' has been removed from version 1.127.0". This parameter is determined by the `instance_class`.
