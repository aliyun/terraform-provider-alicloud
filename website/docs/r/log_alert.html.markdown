...
* `version` - (Optional) The version of alert, new alert is 2.0.
* `type` - (Optional) The type of new alert, new alert is default.
...
* `condition` - (Optional) Conditional expression, such as: count> 100, optional for new alert.
* `dashboard` - (Required, Deprecated) The name of the dashboard associated with the alarm. The name of the instrument cluster associated with the alarm. If there is no such instrument cluster, terraform will help you create an empty instrument cluster, optional for new alert.
* `mute_until` - (Optional) Timestamp, notifications before closing again.
* `throttling` - (Optional, Deprecated) Notification interval, default is no interval. Support number + unit type, for example 60s, 1h.
* `notify_threshold` - (Optional, Deprecated) Notification threshold, which is not notified until the number of triggers is reached. The default is 1.
* `no_data_fire` - (Optional) Switch for whether new alert fires when no data happens, default is false.
* `no_data_severity` - (Optional) when no data happens, the severity of new alert.
* `send_resolved` - (Optional) when new alert is resolved, whether to notify, default is false.
* `query_list` - (Required) Multiple conditions for configured alarm query.
    * `project` - (Optional) Query project.
    * `region` - (Optional) Query project region.
    * `role_arn` - (Optional) Query project store's ARN.
    * `chart_title` - (Required) chart title
    * `logstore` - (Required, Deprecated) Query logstore, use store for new alert.
    * `store` - (Optional) Query store for new alert.
    * `store_type` - (Optional) Query store type for new alert, including log,metric,meta.
...
* `notification_list` - (Required, Deprecated) Alarm information notification list, deprecated for new alert.
...
* `labels` - (Optional) Labels for new alert.
    * `key` - (Optional) Labels's key for new alert.
    * `value` - (Optional) Labels's value for new alert.
* `annoations` - (Optional) Annotations for new alert.
    * `key` - (Optional) Annotations's key for new alert.
    * `value` - (Optional) Annotations's value for new alert.
* `group_configuration` - (Optional) Group configuration for new alert.
    * `type` - (Optional) Group configuration type, including no_group, labels_auto, custom.
    * `fileds` - (Optional) Group configuration's fiels when type is custom.
* `severity_configurations` - (Optional) Severity configuration for new alert.
    * `severity` - (Optional) Severity for new alert, including 2,4,6,8,10 for Report,Low,Medium,High,Critical.
    * `eval_condition` - (Optional) Severity when this condition is met.
        * `condition` - (Optional) Condition for each row.
        * `count_condition` - (Optional) Count's condition for the rows met condition above.
* `jon_configurations` - (Optional) Join configuration for different queries.
    * `type` - (Optional) Join type, including cross_join, inner_join, left_join, right_join, full_join, left_exclude, right_exclude, concat, no_join.
