...
* `buy_resource_ids` - (Optional) The IDs of the purchased IP instances. Separate multiple IDs with commas (,). You can obtain the instance IDs from the response of the DedicatedIpNonePoolList operation.
...
* `update` - (Defaults to 5 mins) Used when update the Dedicated Ip Pool.
