...
-> **NOTE:** Available since v1.80.0, you can only use this resource to manage the custom endpoint. Available since v1.121.0, you also can import the primary endpoint and the cluster endpoint, to modify their ssl status and so on. Available since v1.213.0, when modifying the endpoint of `Cluster` and `Primary`, please use the cluster resource.
...
* `endpoint_type` - (Optional, ForceNew) Type of the endpoint. Creating a new endpoint, it only can be `Custom`. Since v1.213.0, when modifying the endpoint of `Cluster` and `Primary`, please use the cluster resource.
