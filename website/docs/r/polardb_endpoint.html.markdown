...
-> **NOTE:** After v1.80.0 and before v1.120.0, you can only use this resource to manage the custom endpoint. Since v1.120.0, you also can import the primary endpoint and the cluster endpoint, to modify their ssl status and so on. 
...
* `endpoint_type` - (Required & ForceNew before v1.120.0, Optional in v1.120.0+) Type of the endpoint. Before v1.120.0, it only can be `Custom`. since v1.120.0, `Custom`, `Cluster`, `Primary` are valid, default to `Custom`. However when creating a new endpoint, it also only can be `Custom`. 
...
* `ssl_action` - (Optional, Available in v1.120.0+) Specifies how to modify the SSL encryption status. Valid values: `Disable`, `Enable`, `Update`.
* `ssl_connection_string_net_type` - (Optional, Available in v1.120.0+) The network type of the endpoint address.  
...
* `ssl_status` - (Available in v1.120.0+) The status of endpoint SSL encryption. `Enabled`: indicates that SSL encryption is enabled; `Disabled`: indicates that SSL encryption is disabled.
* `ssl_connection_string` - (Available in v1.120.0+) The SSL connection string.
* `ssl_expire_time` - (Available in v1.120.0+) The time when the SSL certificate expires. The time follows the ISO 8601 standard in the yyyy-MM-ddTHH:mm:ssZ format. The time is displayed in UTC.
