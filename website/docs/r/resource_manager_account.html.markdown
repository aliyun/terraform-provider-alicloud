...
* `abandonable_check_id` - (Optional, List, Available since v1.248.0) The ID of the check item that can choose to abandon and continue to perform member deletion.
...
* `resell_account_type` - (Optional, Available since v1.248.0) The identity type of the member. Valid values:
...
* `abandon_able_check_id` - (Deprecated since v1.248.0). Field 'abandon_able_check_id' has been deprecated from provider version 1.248.0. New field 'abandonable_check_id' instead.
