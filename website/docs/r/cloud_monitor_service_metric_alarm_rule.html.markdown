...
* `status` - (Optional, Computed) The enabled status of the alarm rule. Valid values:
  - true: enabled.
  - false: disabled.
* `targets` - (Optional, Set) The alert callback targets. Each target identifies an MNS queue or topic that should receive the alarm payload. See [`targets`](#targets) below.
