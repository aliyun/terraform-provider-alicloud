...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_security_group_rule&spm=docs.r.security_group_rule.example&intl_lang=EN_US)
...
* `source_group_owner_account` - (Optional, ForceNew) The Alibaba Cloud account name (login) of the target security group owner when security groups are authorized across accounts. This parameter is invalid if `cidr_ip` has already been set. Prefer `source_group_owner_id` (the numeric account UID) when available; when both are set, the API uses the ID form and silently ignores the account name.
* `source_group_owner_id` - (Optional, ForceNew) The Alibaba Cloud account ID (numeric UID) of the target security group owner when security groups are authorized across accounts. This is the recommended form of cross-account identification and takes precedence over `source_group_owner_account` when both are set. Write-only: `DescribeSecurityGroupAttribute` does not return this field, so it is not populated back into state on Read or Import.
