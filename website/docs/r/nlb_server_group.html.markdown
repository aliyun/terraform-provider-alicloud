...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_nlb_server_group&spm=docs.r.nlb_server_group.example&intl_lang=EN_US)
...
* `health_check_http_version` - (Optional, Computed, Available since v1.274.0) The HTTP protocol version for health checks. Valid values: `HTTP1.0` (default) and `HTTP1.1`.
