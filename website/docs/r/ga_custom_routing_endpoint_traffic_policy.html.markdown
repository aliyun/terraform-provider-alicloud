...
📚 Need more examples? [VIEW MORE EXAMPLES](https://api.aliyun.com/terraform?activeTab=sample&source=Sample&sourcePath=OfficialSample:alicloud_ga_custom_routing_endpoint_traffic_policy&spm=docs.r.ga_custom_routing_endpoint_traffic_policy.example&intl_lang=EN_US)
