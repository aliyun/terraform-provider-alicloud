...
layout: "alicloud"
page_title: "Alicloud: alicloud_express_connect_router_interface_v2"
description: |-
  Provides a Alicloud Express Connect Router Interface resource.
---

# alicloud_express_connect_router_interface_v2
...
resource "alicloud_express_connect_router_interface_v2" "default" {
  auto_renew                  = "true"
  spec                        = "Mini.2"
...
### Deleting `alicloud_express_connect_router_interface_v2` or removing it from your configuration

The `alicloud_express_connect_router_interface_v2` resource allows you to manage  `fast_link_mode = "true"`  instance, but Terraform cannot destroy it.
...
* `access_point_id` - (Optional, ForceNew, Computed) Access point ID
...
-> **NOTE:**  `InstanceChargeType` is required when the value of the parameter is **PrePaid.
...
* `hc_threshold` - (Optional, Int) Health check threshold. Unit: One. The recommend value is 8. Indicates the number of detection packets sent during the specified health check.
...
* `opposite_interface_owner_id` - (Optional, ForceNew, Computed, Int) Account ID of the peer router interface
* `opposite_region_id` - (Required, ForceNew) Region of the connection peer
* `opposite_router_id` - (Optional, ForceNew, Computed) The ID of the router to which the opposite router interface belongs.
...
* `opposite_interface_id` - Opposite router interface ID
...
```shell
$ terraform import alicloud_express_connect_router_interface_v2.example <id>
```