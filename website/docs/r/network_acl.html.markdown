...
-> **WARNING:** Do not mix the use of the `resources` field in this resource with the separate `alicloud_vpc_network_acl_attachment` resource to bind VSW (Virtual Switch) to the same ACL. Using both methods simultaneously can cause conflicts and result in repeated apply operations that toggle between binding and unbinding VSWs. Choose one method and stick with it to avoid these issues.
