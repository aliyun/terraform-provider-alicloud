...
* `id` - The ID of the PolarDB Application Endpoint.

## Timeouts

The `timeouts` block allows you to specify [timeouts](https://developer.hashicorp.com/terraform/language/resources/syntax#operation-timeouts) for certain actions:

* `create` - (Defaults to 10 mins) Used when creating the polardb application endpoint.
* `delete` - (Defaults to 10 mins) Used when deleting the polardb application endpoint.
...
```shell
$ terraform import alicloud_polardb_application_endpoint.example pa-abc12345678
```