...
* `filter` - (Optional, ForceNew) Custom filter block as described below. See [`filter`](#filter) below.
...
* `tags` - (Optional) A map of tags to filter Virtual Border Routers that match the given tags.
...
* `tags` - The tags of the Virtual Border Router.
