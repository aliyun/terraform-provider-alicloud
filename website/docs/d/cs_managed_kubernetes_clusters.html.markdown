...
* `clusters` - A list of matched Kubernetes clusters. Each element contains the following attributes:
  ...
  * `resource_group_id` - (Optional, Available in 1.99.0+) The ID of the resource group,by default these cloud resources are automatically assigned to the default resource group.
  * `cluster_spec` - (Optional, Available in 1.99.0+) The cluster specifications of kubernetes cluster,which can be empty.Valid values:
