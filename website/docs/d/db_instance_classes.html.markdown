...
* `sorted_by` - (Optional, Available in 1.63.0+) Sort the list of results. Valid value: ["Price"] when the `engine_version` is not empty string.
...
* `instance_classes` - A list of Rds available resource. Each element contains the following attributes:
  ...
  * `price` - The price will be query and assigned when the `engine_version` be specified and `sorted_by` is "Price".
