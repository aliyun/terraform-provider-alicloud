...
func (s *ElasticsearchServiceV2) DescribeElasticsearchLogstash(id string) (object map[string]interface{}, err error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"InstanceNotFound"}) {
			return object, WrapErrorf(NotFoundErr("Logstash", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *ElasticsearchServiceV2) ElasticsearchLogstashStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ElasticsearchLogstashStateRefreshFuncWithApi(id, field, failStates, s.DescribeElasticsearchLogstash)
}

func (s *ElasticsearchServiceV2) ElasticsearchLogstashStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
