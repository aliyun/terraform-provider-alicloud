...
func resourceAliCloudRealtimeComputeVariableRead(d *schema.ResourceData, meta interface{}) error {
	...
	// configured value (Clear/Encrypted); keep the user's configured kind to
	// avoid a perpetual diff.
	// value is Sensitive and masked as "******" by the List API, so it cannot be
	// read back; leave the user's configured value untouched in state.
