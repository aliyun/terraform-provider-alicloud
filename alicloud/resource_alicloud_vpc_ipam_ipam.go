...
func resourceAliCloudVpcIpamIpamCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("operating_region_list"); ok {
		operatingRegionListMaps := v.(*schema.Set).List()
		request["OperatingRegionList"] = operatingRegionListMaps
	}
...
func resourceAliCloudVpcIpamIpamUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && d.HasChange("operating_region_list") {
		...
		if removed.Len() > 0 {
			...
			removeOperatingRegionMaps := localData
			request["RemoveOperatingRegion"] = removeOperatingRegionMaps
		...
		if added.Len() > 0 {
			...
			addOperatingRegionMaps := localData
			request["AddOperatingRegion"] = addOperatingRegionMaps
