	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"mse_version": {
				...
				Computed:     true,
				ForceNew:     true,
				ValidateFunc: StringInSlice([]string{"mse_dev", "mse_pro", "mse_serverless"}, false),
			},
			"vpc_id": {
				Type:     schema.TypeString,
				Optional: true,
...
func resourceAlicloudMseClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	//describeObject, err := mseService.DescribeMseCluster(d.Id())
	//if err != nil {
	//	if !d.IsNewResource() && NotFoundError(err) {
	//		log.Printf("[DEBUG] Resource alicloud_mse_cluster mseService.DescribeMseCluster Failed!!! %s", err)
	//		d.SetId("")
	...
	//	return WrapError(err)
	//}
	//d.Set("cluster_alias_name", describeObject["ClusterAliasName"])
