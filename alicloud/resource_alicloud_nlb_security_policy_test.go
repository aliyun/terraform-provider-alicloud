...
// Case 3995
func TestAccAliCloudNlbSecurityPolicy_basic3995(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_nlb_security_policy.default"
	ra := resourceAttrInit(resourceId, AlicloudNlbSecurityPolicyMap3995)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tf-testacc%snlbsecuritypolicy%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudNlbSecurityPolicyBasicDependence3995)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var AlicloudNlbSecurityPolicyMap3995 = map[string]string{
	"status": CHECKSET,
}

func AlicloudNlbSecurityPolicyBasicDependence3995(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
...
// Case 3995  twin
func TestAccAliCloudNlbSecurityPolicy_basic3995_twin(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_nlb_security_policy.default"
	ra := resourceAttrInit(resourceId, AlicloudNlbSecurityPolicyMap3995)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"security_policy_name": name,
					"ciphers": []string{
						"ECDHE-ECDSA-AES128-SHA", "ECDHE-RSA-AES128-SHA", "TLS_AES_128_GCM_SHA256"},
					"tls_versions": []string{
