...
func resourceAliyunSlbDomainExtensionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(3*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedErrors(err, []string{DomainExtensionProcessing}) {
...
func resourceAliyunSlbDomainExtensionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedErrors(err, []string{DomainExtensionProcessing, InternalError}) {
				return resource.RetryableError(err)
			}
			return resource.NonRetryableError(err)
		}
	...
	if err != nil {
		if IsExceptedErrors(err, []string{InvalidDomainExtensionIdNotFound}) {
			return nil
		}
