...
func resourceAliCloudEsaSiteVersionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("origin_version"); ok {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_esa_site_version", action, AlibabaCloudSdkGoERROR)
	}
...
func resourceAliCloudEsaSiteVersionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_esa_site_version DescribeEsaSiteVersion Failed!!! %s", err)
			d.SetId("")
			return nil
		}
