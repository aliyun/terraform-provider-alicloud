		...
		Schema: map[string]*schema.Schema{
			...
			"notify_strategy": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						// The service ignores updates to this field, so changing it
						...
						"workspace_filter_setting": {
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									// The service accepts tagSelector on write but never persists it, so it
									// cannot be managed declaratively. It is exposed read-only until the
