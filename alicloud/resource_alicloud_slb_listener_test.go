...
func TestAccAlicloudSlbListener_http_redirection(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccSlbListenerHttpRedirection,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSlbListenerExists("alicloud_slb_listener.http", 80),
					resource.TestCheckResourceAttr(
						"alicloud_slb_listener.http", "protocol", "http"),
					resource.TestCheckResourceAttr(
						"alicloud_slb_listener.http", "forward_port", "443"),
					resource.TestCheckResourceAttr(
						"alicloud_slb_listener.http", "listener_forward", "true"),
				),
			},
		},
	})
...
const testAccSlbListenerHttpRedirection = `
resource "alicloud_slb" "instance" {
  name = "tf-testAccSlbListenerHttpRedirection"
  internet_charge_type = "PayByTraffic"
  internet = true
}
resource "alicloud_slb_listener" "http" {
...
variable "name" {
  default = "tf-testAcc-http-redirection-listener-acl"
}
variable "ip_version" {
  default = "ipv4"
}
