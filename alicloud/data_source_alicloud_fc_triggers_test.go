...
var testTriggerLogTemplateDs = `{"sourceConfig": {"logstore": "${alicloud_log_store.bar.name}"},"jobConfig": {"maxRetryTime": 3,"triggerInterval": 60},"functionParameter": {"a": "b","c": "d"},"logConfig": {"project": "${alicloud_log_project.foo.name}","logstore": "${alicloud_log_store.foo.name}"},"enable": true}`
