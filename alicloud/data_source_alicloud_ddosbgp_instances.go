...
func dataSourceAlicloudDdosbgpInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	request.PageNo = "1"
	request.RegionId = d.Get("region").(string)
	request.DdosRegionId = d.Get("region").(string)
	...
	for _, instance := range instances {
		instanceTypeIds = append(instanceTypeIds, instance.InstanceId)
		ipTypeMap[instance.InstanceId] = instance.IpType
	}

	if len(instanceIds) < 1 {
		return WrapError(extractDdosbgpInstance(d, nameMap, ipTypeMap, []ddosbgp.InstanceSpec{}))
	}
	...
	specReq.RegionId = d.Get("region").(string)
	specReq.DdosRegionId = d.Get("region").(string)
	...
	return WrapError(extractDdosbgpInstance(d, nameMap, ipTypeMap, resp.InstanceSpecs))
}

func extractDdosbgpInstance(d *schema.ResourceData, nameMap map[string]string, ipTypeMap map[string]string, instanceSpecs []ddosbgp.InstanceSpec) error {
	var instanceIds []string
	var names []string
