...
func (s *AdbService) DescribeAdbResourcePool(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewAdsClient()
	if err != nil {
		return nil, WrapError(err)
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"PoolNotExist", "InvalidDBCluster.NotFound"}) {
			return object, WrapErrorf(Error(GetNotFoundMessage("ADB:ResourcePool", id)), NotFoundMsg, ProviderERROR, fmt.Sprint(response["RequestId"]))
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
	v, err := jsonpath.Get("$.PoolsInfo", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.PoolsInfo", response)
	}
	if len(v.([]interface{})) < 1 {
		return object, WrapErrorf(Error(GetNotFoundMessage("ADB", id)), NotFoundWithResponse, response)
	} else {
		if fmt.Sprint(v.([]interface{})[0].(map[string]interface{})["PoolName"]) != parts[1] {
			return object, WrapErrorf(Error(GetNotFoundMessage("ADB", id)), NotFoundWithResponse, response)
		}
	}
	object = v.([]interface{})[0].(map[string]interface{})
