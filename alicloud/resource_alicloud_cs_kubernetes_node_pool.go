...
// of sysctl entries cannot be detected, which is documented on the resource). An
// explicitly declared block keeps its state shape: a block whose fields are all unset
// (e.g. `os_config {}` or `os_config { hugepage {} }`) is mirrored back as an empty
// block so the plan converges, while the cloud side receives an empty map (cleared)
// under the API's full-replacement semantics. When the API returns no hugepage keys
// and no os_config block is configured, an empty list is returned so that
// unconfigured node pools keep os_config.# = 0 without a diff.
func flattenOsConfig(d *schema.ResourceData, raw interface{}) []map[string]interface{} {
	configMaps := make([]map[string]interface{}, 0)

	// SDKv1 state materializes an all-zero list element as nil (only the count key is
	// written), so an explicitly declared block is detected through the list length,
	// not through the first element.
	configured := d.Get("os_config")
	configDeclared := osConfigListLen(configured) > 0
	var configFirst map[string]interface{}
	sysctlConfigured := map[string]interface{}{}
	if first := osConfigFirstMap(configured); first != nil {
		configFirst = first
	...
	// Shape mirror only: when the configuration declares a hugepage block whose fields
	// are all unset (or whose element materialized as nil) and the API returns no
	// hugepage keys, one empty element is kept so the state shape matches the
	// configuration. Configured values are never mirrored; the API response stays
	// authoritative for drift detection.
	if len(hugepageMaps) == 0 && configFirst != nil && osConfigListLen(configFirst["hugepage"]) > 0 {
		if hugepageFirst := osConfigFirstMap(configFirst["hugepage"]); hugepageFirst == nil || osConfigHugepageEmpty(hugepageFirst) {
			hugepageMaps = append(hugepageMaps, map[string]interface{}{})
		}
	}

	if len(sysctlConfigured) > 0 || len(hugepageMaps) > 0 || configDeclared {
		configMap := make(map[string]interface{})
		if len(sysctlConfigured) > 0 || configDeclared {
...
// osConfigListLen returns the element count of a TypeList value (0 for any other
// shape).
func osConfigListLen(v interface{}) int {
...
// osConfigHugepageEmpty reports whether a configured hugepage block carries no
// values (all six fields unset or empty strings).
func osConfigHugepageEmpty(m map[string]interface{}) bool {
