...
func resourceAlicloudCSNodePoolUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("kubelet_configuration") {
		...
		kubeletConfig, err := setKubeletConfigParamsForUpdate(d.Get("kubelet_configuration").([]interface{}))
		if err != nil {
			return WrapError(err)
		}
		rolling, err := setRollingPolicy(d.Get("rolling_policy").([]interface{}))
		if err != nil {
			return WrapError(err)
		}
...
func setKubeletConfigParamsForUpdate(l []interface{}) (*roacs.ModifyNodePoolNodeConfigRequestKubeletConfig, error) {
	config := &roacs.ModifyNodePoolNodeConfigRequestKubeletConfig{}
	if len(l) <= 0 || l[0] == nil {
		return nil, nil
	}
	m := l[0].(map[string]interface{})
...
func setRollingPolicy(l []interface{}) (*roacs.ModifyNodePoolNodeConfigRequestRollingPolicy, error) {
	policy := &roacs.ModifyNodePoolNodeConfigRequestRollingPolicy{}
	if len(l) <= 0 || l[0] == nil {
		return nil, nil
	}
	m := l[0].(map[string]interface{})
