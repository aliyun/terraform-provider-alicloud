...
func TestAccAliCloudMongoDBPublicNetworkAddress_basic0(t *testing.T) {
	...
	// test the modification case again after we cached the replicaSets.
	generateModificationCase := func() map[string]interface{} {
		c := map[string]interface{}{
			// connection_strings is []interface{}
			"connection_strings": MapToSlice(cachedReplicaSets, func(k string, v map[string]interface{}) interface{} {
				return map[string]interface{}{
					"role_id":           k,
					"connection_prefix": v["connection_prefix"],
					"connection_port":   v["connection_port"],
				}
			}),
		}
		log.Printf("[DEBUG] modification case: %v", c)
		return c
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccConfig(generateModificationCase()),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"db_instance_id": CHECKSET,
					}),
