...
func resourceAlicloudSelectDBDbClusterDelete(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := BuildStateConf([]string{""}, []string{"ACTIVATION"}, d.Timeout(schema.TimeoutUpdate), 10*time.Second, selectDBService.SelectDBDbClusterStateRefreshFunc(d.Id(), []string{}))
	if _, err := stateConf.WaitForState(); err != nil {
		return WrapErrorf(err, IdMsg, d.Id())
	}
