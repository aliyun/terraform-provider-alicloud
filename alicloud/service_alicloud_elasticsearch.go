...
func logfile(content string) {
	file, err := os.OpenFile("./es_terraform.log", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0644)
	if err != nil {
		log.Fatalf("open es_terraform.log error : %v", err)
	}
	defer file.Close()
	_, err = file.WriteString(content)
	if err != nil {
		log.Fatalf("write es_terraform.log error : %v", err)
	}
}
