		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"grey_tag_route_description": name,
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"grey_tag_route_description": name + "update",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"grey_tag_route_description": name + "update",
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"application_id"},
			},
		},
	})
}
...
func TestAccAlicloudSaeGreyTagRoute_unit(t *testing.T) {
	...
	for key, value := range map[string]interface{}{
		"grey_tag_route_description": "grey_tag_route_description",
	...
	ReadMockResponse := map[string]interface{}{
		"body": map[string]interface{}{
			"Data": map[string]interface{}{
				...
				"Description":    "grey_tag_route_description",
				"ScRules": []interface{}{
	...
	t.Run("UpdateClientAbnormal", func(t *testing.T) {
		...
		err := resourceAlicloudSaeGreyTagRouteUpdate(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	t.Run("UpdateModifySaeGreyTagRouteAttributeAbnormal", func(t *testing.T) {
		...
		for _, key := range []string{"grey_tag_route_description"} {
	...
	t.Run("UpdateModifySaeGreyTagRouteAttributeNoRetryErrorAbnormal", func(t *testing.T) {
		...
		patcheDescribe := gomonkey.ApplyMethod(reflect.TypeOf(&SaeService{}), "DescribeSaeGreyTagRoute", func(*SaeService, string) (map[string]interface{}, error) {
	...
	t.Run("UpdateModifySaeGreyTagRouteAttributeNormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"grey_tag_route_description", "dubbo_rules", "sc_rules"} {
			switch p["alicloud_sae_grey_tag_route"].Schema[key].Type {
			case schema.TypeString:
				diff.SetAttribute(key, &terraform.ResourceAttrDiff{Old: d.Get(key).(string), New: d.Get(key).(string) + "_update"})
			case schema.TypeBool:
