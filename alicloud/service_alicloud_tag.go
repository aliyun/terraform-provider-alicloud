...
func (s *TagService) DescribeTagValue(id string) (object string, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewTagClient()
	if err != nil {
	...
	if !idExist {
		return object, WrapErrorf(Error(GetNotFoundMessage("Tag:MetaTag", id)), NotFoundWithResponse, response)
	}
	return temp1, nil
}
