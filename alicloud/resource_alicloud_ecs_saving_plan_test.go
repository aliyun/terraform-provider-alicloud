...
func TestAccAlicloudEcsSavingPlan_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAlicloudEcsSavingPlanConfigBasic(ra),
...
func TestAccAlicloudEcsSavingPlan_unitTest(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAlicloudEcsSavingPlanConfigGeneral(ra),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAlicloudDataSourceID(resourceName),
...
func testAccAlicloudEcsSavingPlanConfigBasic(ra string) string {
	return fmt.Sprintf(`
resource "alicloud_ecs_saving_plan" "default" {
  ...
  saving_plan_name  = "%s-name"
  description       = "test saving plan"
}
`, ra)
}

func testAccAlicloudEcsSavingPlanConfigGeneral(ra string) string {
	return fmt.Sprintf(`
resource "alicloud_ecs_saving_plan" "default" {
  committed_amount  = "0.08"
  plan_type         = "General"
