...
func resourceAliyunBgpGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if _, ok := d.Get("is_fake_asn").(bool); ok {
	...
	id := response.BgpGroupId
	d.SetId(id)

	if err := BgpGroupService.WaitForBgpGroup(d.Id(), Available, 2*DefaultTimeoutMedium); err != nil {
