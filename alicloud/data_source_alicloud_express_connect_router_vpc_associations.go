...
func dataSourceAliCloudExpressConnectRouterVpcAssociations() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAliCloudExpressConnectRouterVpcAssociationRead,
...
func dataSourceAliCloudExpressConnectRouterVpcAssociationRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_express_connect_router_vpc_associations", action, AlibabaCloudSdkGoERROR)
		}
		...
		for _, v := range result {
			...
			if len(idsMap) > 0 {
				if _, ok := idsMap[fmt.Sprintf("%v:%v:%v", item["EcrId"], item["AssociationId"], item["VpcId"])]; !ok {
					continue
				}
			}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			"id":                    fmt.Sprintf("%v:%v:%v", object["EcrId"], object["AssociationId"], object["VpcId"]),
