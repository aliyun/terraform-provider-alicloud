...
func (s *NlbServiceV2) DescribeNlbLoadbalancerCommonBandwidthPackageAttachment(id string) (object map[string]interface{}, err error) {
	...
	for _, v := range result {
		...
		if item["BandwidthPackageId"] != parts[1] {
			continue
		}
		if item["LoadBalancerId"] != parts[0] {
			continue
		}
		return item, nil
	}
	return object, WrapErrorf(Error(GetNotFoundMessage("LoadbalancerCommonBandwidthPackageAttachment", id)), NotFoundMsg, response)
}
...
func (s *NlbServiceV2) DescribeNlbLoadBalancerSecurityGroupAttachment(id string) (object map[string]interface{}, err error) {
	...
	for _, v := range result {
		...
		if item["SecurityGroupIds[*]"] != parts[1] {
			continue
		}
		return item, nil
	}
