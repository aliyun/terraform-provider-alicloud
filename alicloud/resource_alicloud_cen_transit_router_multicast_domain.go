...
func resourceAliCloudCenTransitRouterMulticastDomainUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("transit_router_multicast_domain_name") {
		update = true
		request["TransitRouterMulticastDomainName"] = d.Get("transit_router_multicast_domain_name")
	}

	if d.HasChange("transit_router_multicast_domain_description") {
		update = true
		request["TransitRouterMulticastDomainDescription"] = d.Get("transit_router_multicast_domain_description")
	}

	if d.HasChange("options.0.igmpv2_support") {
		update = true
		jsonPathResult2, err := jsonpath.Get("$[0].igmpv2_support", d.Get("options"))
		if err == nil {
			request["Options.Igmpv2Support"] = jsonPathResult2
		}
	}
