...
func (s *SlsServiceV2) DescribeSlsLogStore(id string) (object map[string]interface{}, err error) {
	...
	wait := incrementalWait(3*time.Second, 5*time.Second)
	err = resource.Retry(1*time.Minute, func() *resource.RetryError {
		response, err = conn.Execute(genRoaParam("GetLogStore", "GET", "2020-12-30", action), &openapi.OpenApiRequest{Query: query, Body: body, HostMap: hostMap}, &util.RuntimeOptions{})
