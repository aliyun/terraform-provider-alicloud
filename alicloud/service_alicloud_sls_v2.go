...
// DescribeSlsOssIngestion <<< Encapsulated get interface for Sls OssIngestion.

func (s *SlsServiceV2) DescribeSlsOssIngestion(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	query = make(map[string]*string)
	hostMap = make(map[string]*string)
	hostMap["ossIngestionName"] = StringPointer(parts[1])
	hostMap["project"] = StringPointer(parts[0])
	...
	wait := incrementalWait(3*time.Second, 5*time.Second)
	err = resource.Retry(1*time.Minute, func() *resource.RetryError {
		response, err = conn.Execute(genRoaParam("GetOssIngestion", "GET", "2020-12-30", action), &openapi.OpenApiRequest{Query: query, Body: nil, HostMap: hostMap}, &util.RuntimeOptions{})
...
func (s *SlsServiceV2) SlsOssIngestionStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeSlsOssIngestion(id)
		if err != nil {
			if NotFoundError(err) {
				return object, "", nil
			}
...
// DescribeSlsOssIngestion >>> Encapsulated.
