	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"watermark_type": {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: validation.StringInSlice([]string{"EndUserId", "HostName", "Custom"}, false),
			},
		},
	}
}
...
func resourceAlicloudEcdPolicyGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("authorize_security_policy_rules"); ok {
		//authorizeSecurityPolicyRulesMaps := make([]map[string]interface{}, 0)
		for authorizeSecurityPolicyRulesPtr, authorizeSecurityPolicyRules := range v.(*schema.Set).List() {
			...
			//authorizeSecurityPolicyRulesMap := map[string]interface{}{}
	...
	if v, ok := d.GetOk("watermark_custom_text"); ok {
		request["WatermarkCustomText"] = v
	}
...
func resourceAlicloudEcdPolicyGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	//d.Set("preempt_login", object["PreemptLogin"])
	//d.Set("preempt_login_users", object["PreemptLoginUsers"])
	...
	d.Set("watermark_custom_text", object["WatermarkCustomText"])
...
func resourceAlicloudEcdPolicyGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("authorize_access_policy_rules") {
		update = true
		setAuthAccessPolicyRules(d, request, "authorize_access_policy_rules")
		d.SetPartial("authorize_access_policy_rules")
	}
	if d.HasChange("authorize_security_policy_rules") {
		update = true
		setAuthSecurityPolicyRules(d, request, "authorize_security_policy_rules")
	...
	if d.HasChange("watermark_custom_text") {
		update = true
	}
	if v, ok := d.GetOk("watermark_custom_text"); ok {
		request["WatermarkCustomText"] = v
...
func setAuthAccessPolicyRules(d *schema.ResourceData, request map[string]interface{}, resourceType string) {
	if d.HasChange(resourceType) {
		...
		if len(removed) > 0 {
			for authorizeAccessPolicyRulesPtr, authorizeAccessPolicyRules := range removed {
				authorizeAccessPolicyRulesArg := authorizeAccessPolicyRules.(map[string]interface{})
				request["RevokeAccessPolicyRule."+fmt.Sprint(authorizeAccessPolicyRulesPtr+1)+".CidrIp"] = authorizeAccessPolicyRulesArg["cidr_ip"]
				request["RevokeAccessPolicyRule."+fmt.Sprint(authorizeAccessPolicyRulesPtr+1)+".Description"] = authorizeAccessPolicyRulesArg["description"]
			}
...
func setAuthSecurityPolicyRules(d *schema.ResourceData, request map[string]interface{}, resourceType string) {
	if d.HasChange(resourceType) {
		removed, added := parsingResourceType(d, resourceType)
		if len(removed) > 0 {
			for authorizeSecurityPolicyRulesPtr, authorizeSecurityPolicyRules := range removed {
				authorizeSecurityPolicyRulesArg := authorizeSecurityPolicyRules.(map[string]interface{})
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".CidrIp"] = authorizeSecurityPolicyRulesArg["cidr_ip"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".Description"] = authorizeSecurityPolicyRulesArg["description"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".IpProtocol"] = authorizeSecurityPolicyRulesArg["ip_protocol"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".Policy"] = authorizeSecurityPolicyRulesArg["policy"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".PortRange"] = authorizeSecurityPolicyRulesArg["port_range"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".Priority"] = authorizeSecurityPolicyRulesArg["priority"]
				request["RevokeSecurityPolicyRule."+fmt.Sprint(authorizeSecurityPolicyRulesPtr+1)+".Type"] = authorizeSecurityPolicyRulesArg["type"]
			}
