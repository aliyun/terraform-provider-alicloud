...
// Case 通用型性能型文件系统（SMB_ACL） 10581
func TestAccAliCloudNasFileSystem_basic10581(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_nas_file_system.default"
	ra := resourceAttrInit(resourceId, AlicloudNasFileSystemMap10581)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &NasServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeNasFileSystem")
	rac := resourceAttrCheckInit(rc, ra)
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudNasFileSystemBasicDependence10581)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
...
var AlicloudNasFileSystemMap10581 = map[string]string{
	"status":      CHECKSET,
	"create_time": CHECKSET,
	"region_id":   CHECKSET,
}

func AlicloudNasFileSystemBasicDependence10581(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
