...
func TestAccAlicloudEcsDisksDataSource(t *testing.T) {
	...
	availabilityZoneConf := dataSourceTestAccConfig{
		existConfig: testAccCheckAlicloudEcsDisksDataSourceName(rand, map[string]string{
			"ids":               `["${alicloud_ecs_disk.default.id}"]`,
			"availability_zone": `"${alicloud_ecs_disk.default.availability_zone}"`,
		}),
	}
	...
	alicloudEcsDisksCheckInfo.dataSourceTestCheck(t, rand, idsConf, availabilityZoneConf, categoryConf, deleteAutoSnapshotConf, deleteWithInstanceConf, diskNameConf, enableAutoSnapshotConf, encryptedConf, paymentTypeConf, tagsConf, nameRegexConf, statusConf, allConf)
}
