...
func resourceAliCloudMilvusInstance() *schema.Resource {
	return &schema.Resource{
		...
		StateUpgraders: []schema.StateUpgrader{
			{
				Version: 0,
				Type:    resourceAliCloudMilvusInstanceV0().CoreConfigSchema().ImpliedType(),
				Upgrade: resourceAliCloudMilvusInstanceStateUpgradeV0,
			},
		},
...
func resourceAliCloudMilvusInstanceV0() *schema.Resource {
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"auto_backup": {
				Type:     schema.TypeBool,
...
func resourceAliCloudMilvusInstanceStateUpgradeV0(rawState map[string]interface{}, _ interface{}) (map[string]interface{}, error) {
	if v, ok := rawState["components"]; ok && v != nil {
		switch val := v.(type) {
		case map[string]interface{}:
			if len(val) > 0 {
				rawState["components"] = []interface{}{val}
			} else {
				rawState["components"] = []interface{}{}
			}
		}
	}
	return rawState, nil
