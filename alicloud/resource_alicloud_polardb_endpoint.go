...
const polardbEndpointSuffixRegex = "\\.[a-zA-Z0-9\\-\\.]+\\.rds\\.aliyuncs\\.com"
const polardbEndpointIdWithSuffixRegex = "^([a-zA-Z0-9\\-_]+:[a-zA-Z0-9\\-_]+:[a-zA-Z0-9\\-_]+)" + polardbEndpointSuffixRegex + "$"

var polardbEndpointIdWithSuffixRegexp = regexp.MustCompile(polardbEndpointIdWithSuffixRegex)
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"net_type": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validation.StringInSlice([]string{string("Public"), string("Private")}, false),
				Default:      "Public",
				ForceNew:     true,
			},
			"connection_prefix": {
...
func resourceAlicloudPolarDBEndpointRead(d *schema.ResourceData, meta interface{}) error {
	submatch := polardbEndpointIdWithSuffixRegexp.FindStringSubmatch(d.Id())
	if len(submatch) > 1 {
		d.SetId(submatch[1])
	}
