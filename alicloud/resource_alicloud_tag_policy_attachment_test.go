	...
	for {
		...
		for _, v := range result {
			...
			for _, w := range result1 {
				...
				attachmentIds = append(attachmentIds, fmt.Sprintf("%v:%v:%v:%v:%v", item["PolicyId"], item1["TargetId"], item1["TargetType"]))
			}
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"policy_id":   "${alicloud_tag_policy.this.policy_id}",
					"target_id":   "1619897336813314",
					"target_type": "USER",
				}),
