	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"status": {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: StringInSlice([]string{"Cancelling", "RollbackInProgress", "Paused", "InProgress"}, false),
			},
			"checkpoints": {
				Optional: true,
				ForceNew: true,
...
func resourceAliyunEssInstanceRefreshUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	update := false
	var status string
	if d.HasChange("status") && (d.Get("status").(string) == "Cancelling" || d.Get("status").(string) == "RollbackInProgress" || d.Get("status").(string) == "Paused" || d.Get("status").(string) == "InProgress") {
