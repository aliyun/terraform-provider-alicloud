...
func resourceAliCloudCloudMonitorServiceMetricAlarmRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	// `targets` is managed by the standalone PutMetricRuleTargets /
	// DeleteMetricRuleTargets APIs and is not part of PutResourceMetricRule's
	// payload, so CMS keeps target records independent of the rule definition.
	// Only refresh the field when the user is actually managing targets via this
	// resource — pulling them in unconditionally would introduce spurious plan
	// diffs for users who never configured a target (stale rows from earlier
	// callers on the same RuleId would otherwise appear in state).
	if v, ok := d.GetOk("targets"); ok && len(v.([]interface{})) > 0 {
		objectRaw, err = cloudMonitorServiceServiceV2.DescribeMetricAlarmRuleDescribeMetricRuleTargets(d.Id())
		if err != nil && !NotFoundError(err) {
			return WrapError(err)
		}

		targetRaw, _ := jsonpath.Get("$.Targets.Target", objectRaw)
		if targetRaw != nil && len(convertToInterfaceArray(targetRaw)) > 0 {
			targetsMaps := make([]map[string]interface{}, 0)
			for _, targetChildRaw := range convertToInterfaceArray(targetRaw) {
				targetsMap := make(map[string]interface{})
				targetChildRaw := targetChildRaw.(map[string]interface{})
				targetsMap["arn"] = targetChildRaw["Arn"]
				targetsMap["json_params"] = targetChildRaw["JsonParams"]
				targetsMap["level"] = targetChildRaw["Level"]
				targetsMap["target_id"] = targetChildRaw["Id"]
...
func resourceAliCloudCloudMonitorServiceMetricAlarmRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	// PutMetricRuleTargets is a full-replace PUT on the rule's target set: send
	// the full target list whenever the user manages targets in config. We
	// can't rely on d.HasChange("targets") here because `targets` is
	// Optional+Computed, which makes HasChange unreliable when only nested
	// leaves (e.g. json_params) change. Always forwarding the current config
	// keeps server-side state in sync with what the user declared.
	if v, ok := d.GetOk("targets"); ok {
		targetsMapsArray := make([]interface{}, 0)
		for _, dataLoop := range convertToInterfaceArray(v) {
			dataLoopTmp := dataLoop.(map[string]interface{})
			dataLoopMap := make(map[string]interface{})
			dataLoopMap["Arn"] = dataLoopTmp["arn"]
			dataLoopMap["JsonParams"] = dataLoopTmp["json_params"]
			dataLoopMap["Id"] = dataLoopTmp["target_id"]
			dataLoopMap["Level"] = dataLoopTmp["level"]
			targetsMapsArray = append(targetsMapsArray, dataLoopMap)
		}
