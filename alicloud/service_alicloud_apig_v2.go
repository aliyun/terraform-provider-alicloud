...
// DescribeApigPortal <<< Encapsulated get interface for Apig Portal.

func (s *ApigServiceV2) DescribeApigPortal(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	addDebug(action, response, request)
	if response == nil {
		return object, WrapErrorf(NotFoundErr("Portal", id), NotFoundMsg, response)
	}
	if err != nil {
		if IsExpectedErrors(err, []string{"NOT_FOUND"}) {
			return object, WrapErrorf(NotFoundErr("Portal", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *ApigServiceV2) ApigPortalStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ApigPortalStateRefreshFuncWithApi(id, field, failStates, s.DescribeApigPortal)
}

func (s *ApigServiceV2) ApigPortalStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeApigPortal >>> Encapsulated.
