  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "Service": ["cloudmonitor.aliyuncs.com"]
      }
    }
  ]
}
...
resource "alicloud_resource_manager_resource_group" "default" {
  resource_group_name = "tf-acc-cms-de-rg-%[1]s"
  display_name        = "tf-acc-cms-de-rg-%[1]s"
}
`, name)
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceId, "resource_group_id", "alicloud_resource_manager_resource_group.default", "id"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceId, "digital_employee_name", name),
					resource.TestCheckResourceAttr(resourceId, "tags.#", "1"),
					testAccCheckCmsDigitalEmployeeTag(resourceId, "env-updated", "tf-acc-updated"),
				),
			},
			{
...
// testAccCheckCmsDigitalEmployeeTag asserts that a TypeSet tags block contains
// the given key/value pair. terraform-plugin-sdk v1 has no
// TestCheckTypeSetElemNestedAttrs, so the set element is located by scanning
// state attributes for a tags.<hash>.key match.
func testAccCheckCmsDigitalEmployeeTag(resourceId, key, value string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for attr, v := range rs.Primary.Attributes {
			if strings.HasPrefix(attr, "tags.") && strings.HasSuffix(attr, ".key") && v == key {
				valueAttr := strings.TrimSuffix(attr, ".key") + ".value"
				if rs.Primary.Attributes[valueAttr] == value {
					return nil
				}
			}
		}
		return fmt.Errorf("tag %s=%s not found in %s state attributes", key, value, resourceId)
	}
}
...
// TestAccAliCloudCmsDigitalEmployee_knowledgesBailianOrder verifies that
// reordering the knowledges.bailian TypeList produces a non-empty plan and
// then converges after applying the reordered configuration.
func TestAccAliCloudCmsDigitalEmployee_knowledgesBailianOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
					...
					"knowledges": []map[string]interface{}{
						{
							"bailian": []map[string]interface{}{
								{
									"workspace_id": "ws-a-" + name,
									"index_id":     "idx-a-" + name,
									"region":       "cn-beijing",
									"attributes":   hclEscapeString(`{"source":"a"}`),
								},
								{
									"workspace_id": "ws-b-" + name,
									"index_id":     "idx-b-" + name,
									"region":       "cn-shanghai",
									"attributes":   hclEscapeString(`{"source":"b"}`),
								},
							},
						},
					},
...
// TestAccAliCloudCmsDigitalEmployee_allowCidrsOrder verifies that reordering
// the sandbox_network_policy.allow_cidrs TypeList produces a non-empty plan
// and then converges.
func TestAccAliCloudCmsDigitalEmployee_allowCidrsOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"sandbox_network_policy": []map[string]interface{}{
						{
							"allow_fqdns": []string{"api.example.com", "cdn.example.com"},
							"allow_cidrs": []string{"192.168.1.1", "10.0.0.0/8"},
							"enable_acl":  false,
						},
					},
					"tool_policy": []map[string]interface{}{
...
// TestAccAliCloudCmsDigitalEmployee_allowFqdnsOrder verifies that reordering
// the sandbox_network_policy.allow_fqdns TypeList produces a non-empty plan
// and then converges.
func TestAccAliCloudCmsDigitalEmployee_allowFqdnsOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
		...
		Steps: []resource.TestStep{
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"sandbox_network_policy": []map[string]interface{}{
						{
							"allow_fqdns": []string{"cdn.example.com", "api.example.com"},
							"allow_cidrs": []string{"10.0.0.0/8", "192.168.1.1"},
							"enable_acl":  false,
						},
					},
...
// TestAccAliCloudCmsDigitalEmployee_autoPassPolicyOrder verifies that
// reordering the tool_policy.aliyun.auto_pass_policy TypeList produces a
// non-empty plan and then converges.
func TestAccAliCloudCmsDigitalEmployee_autoPassPolicyOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
							...
							"aliyun": []map[string]interface{}{
								{
									"enable":           true,
									"deny_policy":      []string{"ecs:Delete*"},
									"auto_pass_policy": []string{"log:List*", "log:Get*"},
									"statements": []map[string]interface{}{
										{
											"decision":    "user_ack",
											"product":     "Sls",
...
// TestAccAliCloudCmsDigitalEmployee_denyPolicyOrder verifies that reordering
// the tool_policy.aliyun.deny_policy TypeList produces a non-empty plan and
// then converges.
func TestAccAliCloudCmsDigitalEmployee_denyPolicyOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
					...
					"tool_policy": []map[string]interface{}{
						{
							"aliyun": []map[string]interface{}{
								{
									"enable":           true,
									"deny_policy":      []string{"vpc:DeleteVpc", "ecs:Delete*"},
									"auto_pass_policy": []string{"log:Get*", "log:List*"},
									"statements": []map[string]interface{}{
										{
											"decision":    "user_ack",
...
// TestAccAliCloudCmsDigitalEmployee_statementsOrder verifies that reordering
// the tool_policy.aliyun.statements TypeList produces a non-empty plan and
// then converges.
func TestAccAliCloudCmsDigitalEmployee_statementsOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
...
// TestAccAliCloudCmsDigitalEmployee_statementsActionsOrder verifies that
// reordering the tool_policy.aliyun.statements.actions TypeList (while keeping
// the statements themselves in the same order) produces a non-empty plan and
// then converges.
func TestAccAliCloudCmsDigitalEmployee_statementsActionsOrder(t *testing.T) {
	resourceId := "alicloud_cms_digital_employee.default"
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccms%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, testAccCmsDigitalEmployeeDependence)
							...
							"aliyun": []map[string]interface{}{
								{
									...
									"statements": []map[string]interface{}{
										...
										},
										{
											...
											"product":     "Ecs",
											"api_version": "2014-05-26",
											"actions":     []string{"ecs:DescribeInstances", "ecs:DescribeRegions"},
										},
									},
								},
							},
									...
									"statements": []map[string]interface{}{
										{
											...
											"product":     "Sls",
											"api_version": "2020-12-30",
											"actions":     []string{"log:CreateDashboard", "log:GetProject"},
										},
										{
											"decision":    "allow",
											"product":     "Ecs",
											"api_version": "2014-05-26",
											"actions":     []string{"ecs:DescribeRegions", "ecs:DescribeInstances"},
										},
									},
								},
							},
