...
// DescribeEsaAigwInstance <<< Encapsulated get interface for Esa AigwInstance.

func (s *EsaServiceV2) DescribeEsaAigwInstance(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidAIGWInstance.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("AigwInstance", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *EsaServiceV2) EsaAigwInstanceStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeEsaAigwInstance(id)
		if err != nil {
			if NotFoundError(err) {
				return object, "", nil
			}
...
// DescribeEsaAigwInstance >>> Encapsulated.
