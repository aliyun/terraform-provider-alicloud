...
// DescribePaiWorkspaceDefaultWorkspace <<< Encapsulated get interface for PaiWorkspace DefaultWorkspace.

func (s *PaiWorkspaceServiceV2) DescribePaiWorkspaceDefaultWorkspace(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
...
func (s *PaiWorkspaceServiceV2) PaiWorkspaceDefaultWorkspaceStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribePaiWorkspaceDefaultWorkspace(id)
		if err != nil {
			if NotFoundError(err) {
				return object, "", nil
			}
...
// DescribePaiWorkspaceDefaultWorkspace >>> Encapsulated.
