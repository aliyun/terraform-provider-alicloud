	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"instance_name": {
				...
				Optional:      true,
				Computed:      true,
				Removed:       "Field 'instance_name' has been removed from version 1.127.0. Use 'db_instance_name' instead.",
			...
			"node_type": {
				...
				Removed:      "Field 'node_type' has been removed from version 1.127.0",
			},
			"order_type": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"instance_charge_type": {
				...
				Computed:      true,
				ValidateFunc:  validation.StringInSlice([]string{"PostPaid", "PrePaid"}, false),
				Removed:       "Field 'instance_charge_type' has been removed from version 1.127.0. Use 'payment_type' instead.",
			...
			"availability_zone": {
				...
				Optional:      true,
				Computed:      true,
				Removed:       "Field 'availability_zone' has been removed from version 1.127.0. Use 'zone_id' instead.",
			...
			"enable_public": {
				...
				Optional: true,
				Computed: true,
				Removed:  "Field 'enable_public' has been removed from version 1.127.0. Please use resource 'alicloud_kvstore_connection' instead.",
			},
			"connection_string_prefix": {
				Type:     schema.TypeString,
				Optional: true,
				Removed:  "Field 'connection_string_prefix' has been removed from version 1.127.0. Please use resource 'alicloud_kvstore_connection' instead.",
			},
			"connection_string": {
				Type:     schema.TypeString,
				Computed: true,
				Removed:  "Field 'connection_string' has been removed from version 1.127.0. Please use resource 'alicloud_kvstore_connection' instead.",
			},
			"parameters": {
				Type: schema.TypeSet,
				...
				Optional:      true,
				Computed:      true,
				Removed:       "Field 'parameters' has been removed from version 1.127.0. Use 'config' instead.",
...
func resourceAlicloudKvstoreInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	for _, instanceNetInfo := range net {
		...
		if instanceData["DBInstanceNetType"].(string) == "2" {
			d.Set("private_connection_port", instanceData["Port"])
		}
	}
	...
	if _, ok := d.GetOk("security_group_id"); ok {
		...
		for _, sg := range describeSecurityGroupConfigurationObject {
			sgData := sg.(map[string]interface{})
			sgs = append(sgs, sgData["SecurityGroupId"].(string))
		}
...
func resourceAlicloudKvstoreInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("private_connection_port") {
		update = true
		modifyDBInstanceConnectionStringReq["Port"] = d.Get("private_connection_port")
	}
