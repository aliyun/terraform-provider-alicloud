...
func resourceAliCloudCloudFirewallPrivateDnsUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("domain_name_list"); ok || d.HasChange("domain_name_list") {
		domainNameListMapsArray := convertToInterfaceArray(v)

		request["DomainNameList"] = domainNameListMapsArray
	}
