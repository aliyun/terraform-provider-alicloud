...
func resourceAliCloudInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && (d.HasChange("system_disk_size") || d.HasChange("system_disk_auto_snapshot_policy_id") || d.HasChange("system_disk_name") || d.HasChange("system_disk_description") || d.HasChange("system_disk_performance_level") || d.HasChange("system_disk_category")) {
		disk, err := ecsService.DescribeEcsSystemDisk(d.Id())
		if err != nil {
			return WrapError(err)
		}

		if d.HasChange("system_disk_performance_level") || d.HasChange("system_disk_category") {
			...
			if d.HasChange("system_disk_category") {
				request["DiskCategory"] = d.Get("system_disk_category")
			}
			wait := incrementalWait(3*time.Second, 3*time.Second)
			err = resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
				response, err = client.RpcPost("Ecs", "2014-05-26", action, nil, request, false)
			...
			stateConf := BuildStateConf([]string{}, []string{"Available", "In_use"}, d.Timeout(schema.TimeoutUpdate), 10*time.Second, ecsService.EcsDiskStateRefreshFunc(d.Id(), []string{}))
			if _, err := stateConf.WaitForState(); err != nil {
				return WrapErrorf(err, IdMsg, d.Id())
			}
		}
