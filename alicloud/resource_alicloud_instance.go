...
func resourceAliCloudInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if value := instanceAttribute["EnableJumboFrame"]; value != nil {
		var ok bool
		enableJumboFrame, ok = value.(bool)
		if !ok {
			return WrapError(fmt.Errorf("DescribeInstanceAttribute returned unexpected EnableJumboFrame type %T", value))
		}
	}
	...
	if value := instanceAttribute["EnableNetworkEncryption"]; value != nil {
		enableNetworkEncryption, ok := value.(bool)
		if !ok {
			return WrapError(fmt.Errorf("DescribeInstanceAttribute returned unexpected EnableNetworkEncryption type %T", value))
		}
		if err := d.Set("enable_network_encryption", enableNetworkEncryption); err != nil {
			return WrapError(err)
		}
