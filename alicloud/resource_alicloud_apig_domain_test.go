...
// Case domain_https_full_update
// Skipped: APIG pre environment rejects protocol=HTTPS with "InvalidParameter.WithValue" —
// once the upstream validation is fixed (or the resource is verified in production), this
// test can be re-enabled to cover the HTTPS-only attributes: force_https, cert_identifier,
// ca_cert_identifier, client_ca_cert, m_tls_enabled, http2_option, tls_min/max, and
// tls_cipher_suites_config value combinations.
func TestAccAliCloudApigDomain_basic12906(t *testing.T) {
	t.Skip("APIG pre environment rejects protocol=HTTPS; see comment above")
	var v map[string]interface{}
	resourceId := "alicloud_apig_domain.default"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
							...
							"tls_cipher_suite": []map[string]interface{}{
								{
									...
									"support_versions": []string{"TLSv1.2", "TLSv1.3"},
								},
							},
						},
					},
			...
			},
			{
							...
							"tls_cipher_suite": []map[string]interface{}{
								{
									...
									"support_versions": []string{"TLSv1.2"},
								},
							},
						},
					},
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"tls_cipher_suites_config.0.config_type": "Default",
			...
			},
			{
				...
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"gateway_type", "client_ca_cert"},
			},
		},
	})
}
