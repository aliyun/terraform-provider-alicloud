...
func resourceAliCloudPolarDbGlobalSecurityIpGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_polar_db_global_security_ip_group", action, AlibabaCloudSdkGoERROR)
	}
...
func resourceAliCloudPolarDbGlobalSecurityIpGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_polar_db_global_security_ip_group DescribePolarDbGlobalSecurityIpGroup Failed!!! %s", err)
			d.SetId("")
			return nil
		}
