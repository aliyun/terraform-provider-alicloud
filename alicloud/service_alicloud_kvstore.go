...
func (s *KvstoreService) DescribeRKVInstanceById(id string) (instance *r_kvstore.DBInstanceAttribute, err error) {
	...
	if err != nil {
		if IsExceptedError(err, InvalidKVStoreInstanceIdNotFound) {
			return nil, WrapErrorf(Error(GetNotFoundMessage("RKVInstance", id)), NotFoundMsg, AlibabaCloudSdkGoERROR)
		}
...
func (s *KvstoreService) DescribeRKVInstanceBackupPolicy(id string) (response *r_kvstore.DescribeBackupPolicyResponse, err error) {
	...
	if err != nil {
		if IsExceptedError(err, InvalidKVStoreInstanceIdNotFound) {
			return nil, WrapErrorf(Error(GetNotFoundMessage("RKVInstanceBackupPolicy", id)), NotFoundMsg, AlibabaCloudSdkGoERROR)
		}
		return nil, WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), AlibabaCloudSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw)
...
func (s *KvstoreService) WaitForRKVInstance(id string, status Status, timeout int) error {
	deadline := time.Now().Add(time.Duration(timeout) * time.Second)
	for {
		object, err := s.DescribeRKVInstanceById(id)
...
func (s *KvstoreService) WaitForRKVInstanceVpcAuthMode(id string, status string, timeout int) error {
	deadline := time.Now().Add(time.Duration(timeout) * time.Second)
	for {
		object, err := s.DescribeRKVInstanceById(id)
		if err != nil && !NotFoundError(err) {
...
func (s *KvstoreService) ModifyInstanceConfig(id string, config string) error {
	...
	if err := s.WaitForRKVInstance(id, Normal, DefaultLongTimeout); err != nil {
