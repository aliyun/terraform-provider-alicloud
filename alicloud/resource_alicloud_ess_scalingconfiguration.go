...
func resourceAliyunEssScalingConfigurationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedErrors(err, []string{EssThrottling, IncorrectScalingGroupStatus}) {
...
func resourceAliyunEssScalingConfigurationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if IsExceptedErrors(err, []string{InvalidScalingGroupIdNotFound, InvalidEssScalingConfigurationIdNotFound}) {
			return nil
		}
		return WrapErrorf(err, DefaultErrorMsg, d.Id(), request.GetActionName(), AlibabaCloudSdkGoERROR)
	}
