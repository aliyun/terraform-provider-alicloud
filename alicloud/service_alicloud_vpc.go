...
func (s *VpcService) DescribeVpnGateways(id string) (object map[string]interface{}, err error) {
	...
	for {
		...
		v, err := jsonpath.Get("$.VpnGateways.VpnGateway", response)
		if err != nil {
			return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.VpnGateways.VpnGateway", response)
		}
		if len(v.([]interface{})) < 1 {
			return object, WrapErrorf(Error(GetNotFoundMessage("VPC", id)), NotFoundWithResponse, response)
		}
		for _, v := range v.([]interface{}) {
			if fmt.Sprint(v.(map[string]interface{})["VpnGatewayId"]) == id {
				idExist = true
				return v.(map[string]interface{}), nil
			}
		}
