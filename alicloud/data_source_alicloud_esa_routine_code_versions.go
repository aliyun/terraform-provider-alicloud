...
func dataSourceAliCloudEsaRoutineCodeVersionsRead(d *schema.ResourceData, meta interface{}) error {
	...
	// ListRoutineCodeVersions only supports PageNumber 1 and 2 (about 40 versions).
	// Iterate until the accumulated count reaches TotalCount or the hard page cap.
	for {
		...
		pageNumber := formatInt(request["PageNumber"])
		if pageNumber >= 2 {
			break
		}
	...
	for _, object := range objects {
		...
		mapping := map[string]interface{}{
			...
			"build_id":         formatInt(object["BuildId"]),
			"has_env_vars":     object["HasEnvVars"],
		}
