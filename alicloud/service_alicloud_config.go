...
func (s *ConfigService) DescribeConfigAggregator(id string) (object map[string]interface{}, err error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"AccountNotExisted", "Invalid.AggregatorId.Value"}) {
...
func (s *ConfigService) ConfigAggregatorStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		for _, failState := range failStates {
			if string(object["AggregatorStatus"].(json.Number)) == failState {
				return object, string(object["AggregatorStatus"].(json.Number)), WrapError(Error(FailedToReachTargetStatus, string(object["AggregatorStatus"].(json.Number))))
			}
		}
		return object, string(object["AggregatorStatus"].(json.Number)), nil
	}
}
