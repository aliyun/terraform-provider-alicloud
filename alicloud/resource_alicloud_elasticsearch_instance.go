...
func resourceAliCloudElasticsearchInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v := d.Get("client_node_configuration"); !IsNil(v) {
		...
		request["clientNodeConfiguration"] = dataList
	}
	...
	if v := d.Get("kibana_configuration"); !IsNil(v) {
		...
		if disk9 != nil && disk9 != "" {
			dataList6["disk"] = disk9
		}
		amount7, _ := jsonpath.Get("$[0].amount", v)
		if amount7 != nil && amount7 != "" {
			dataList6["amount"] = amount7
		}
		spec9, _ := jsonpath.Get("$[0].spec", v)
		if spec9 != nil && spec9 != "" {
			dataList6["spec"] = spec9
		}

		request["kibanaConfiguration"] = dataList6
	}
...
func resourceAliCloudElasticsearchInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if checkValue00 == "Subscription" {
		objectRaw, err = elasticsearchServiceV2.DescribeInstanceQueryAvailableInstances(d.Id())
...
func resourceAliCloudElasticsearchInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("protocol") {
		...
		if objectRaw["protocol"].(string) != target {
			if target == "HTTPS" {
				InstanceId := d.Id()
	...
	if d.HasChange("enable_kibana_private_network") {
		...
		if objectRaw["enableKibanaPrivateNetwork"].(bool) != target {
	...
	if d.HasChange("enable_public") {
		...
		if objectRaw["enablePublic"].(bool) != target {
			if target == true {
				InstanceId := d.Id()
				action := fmt.Sprintf("/openapi/instances/%s/actions/network-trigger", InstanceId)
				request = make(map[string]interface{})
	...
	if d.HasChange("enable_kibana_public_network") {
		...
		if objectRaw["enableKibanaPublicNetwork"].(bool) != target {
			if target == true {
				InstanceId := d.Id()
				action := fmt.Sprintf("/openapi/instances/%s/actions/network-trigger", InstanceId)
				request = make(map[string]interface{})
	...
	if v := d.Get("client_node_configuration"); v != nil {
		disk1, _ := jsonpath.Get("$[0].disk", v)
		if disk1 != nil && (d.HasChange("client_node_configuration.0.disk") || disk1 != "") {
			dataList["disk"] = disk1
		}
		amount1, _ := jsonpath.Get("$[0].amount", v)
		if amount1 != nil && (d.HasChange("client_node_configuration.0.amount") || amount1 != "") {
			dataList["amount"] = amount1
		}
		diskType1, _ := jsonpath.Get("$[0].disk_type", v)
		if diskType1 != nil && (d.HasChange("client_node_configuration.0.disk_type") || diskType1 != "") {
			dataList["diskType"] = diskType1
		}
		spec1, _ := jsonpath.Get("$[0].spec", v)
		if spec1 != nil && (d.HasChange("client_node_configuration.0.spec") || spec1 != "") {
			dataList["spec"] = spec1
		}
	...
	if v := d.Get("data_node_configuration"); v != nil {
		disk3, _ := jsonpath.Get("$[0].disk", v)
		if disk3 != nil && (d.HasChange("data_node_configuration.0.disk") || disk3 != "") {
			dataList1["disk"] = disk3
		}
		diskType3, _ := jsonpath.Get("$[0].disk_type", v)
		if diskType3 != nil && (d.HasChange("data_node_configuration.0.disk_type") || diskType3 != "") {
			dataList1["diskType"] = diskType3
		}
		spec3, _ := jsonpath.Get("$[0].spec", v)
		if spec3 != nil && (d.HasChange("data_node_configuration.0.spec") || spec3 != "") {
			dataList1["spec"] = spec3
		}
		performanceLevel1, _ := jsonpath.Get("$[0].performance_level", v)
		if performanceLevel1 != nil && (d.HasChange("data_node_configuration.0.performance_level") || performanceLevel1 != "") {
			dataList1["performanceLevel"] = performanceLevel1
		}
		diskEncryption1, _ := jsonpath.Get("$[0].disk_encryption", v)
		if diskEncryption1 != nil && (d.HasChange("data_node_configuration.0.disk_encryption") || diskEncryption1 != "") {
			dataList1["diskEncryption"] = diskEncryption1
		}
	...
	if v := d.Get("warm_node_configuration"); v != nil {
		spec5, _ := jsonpath.Get("$[0].spec", v)
		if spec5 != nil && (d.HasChange("warm_node_configuration.0.spec") || spec5 != "") {
			dataList2["spec"] = spec5
		}
		amount3, _ := jsonpath.Get("$[0].amount", v)
		if amount3 != nil && (d.HasChange("warm_node_configuration.0.amount") || amount3 != "") {
			dataList2["amount"] = amount3
		}
		diskEncryption3, _ := jsonpath.Get("$[0].disk_encryption", v)
		if diskEncryption3 != nil && (d.HasChange("warm_node_configuration.0.disk_encryption") || diskEncryption3 != "") {
			dataList2["diskEncryption"] = diskEncryption3
		}
		diskType5, _ := jsonpath.Get("$[0].disk_type", v)
		if diskType5 != nil && (d.HasChange("warm_node_configuration.0.disk_type") || diskType5 != "") {
			dataList2["diskType"] = diskType5
		}
		disk5, _ := jsonpath.Get("$[0].disk", v)
		if disk5 != nil && (d.HasChange("warm_node_configuration.0.disk") || disk5 != "") {
			dataList2["disk"] = disk5
		}

		request["warmNodeConfiguration"] = dataList2
	}
	...
	if v := d.Get("master_configuration"); v != nil {
		disk7, _ := jsonpath.Get("$[0].disk", v)
		if disk7 != nil && (d.HasChange("master_configuration.0.disk") || disk7 != "") {
			dataList3["disk"] = disk7
		}
		amount5, _ := jsonpath.Get("$[0].amount", v)
		if amount5 != nil && (d.HasChange("master_configuration.0.amount") || amount5 != "") {
			dataList3["amount"] = amount5
		}
		spec7, _ := jsonpath.Get("$[0].spec", v)
		if spec7 != nil && (d.HasChange("master_configuration.0.spec") || spec7 != "") {
			dataList3["spec"] = spec7
		}
		diskType7, _ := jsonpath.Get("$[0].disk_type", v)
		if diskType7 != nil && (d.HasChange("master_configuration.0.disk_type") || diskType7 != "") {
			dataList3["diskType"] = diskType7
		}

		request["masterConfiguration"] = dataList3
	}

	if !d.IsNewResource() && d.HasChange("data_node_configuration.0.amount") {
		update = true
	}
	if v, ok := d.GetOk("data_node_configuration"); ok || d.HasChange("data_node_configuration") {
	...
	if v := d.Get("kibana_configuration"); v != nil {
		disk9, _ := jsonpath.Get("$[0].disk", v)
		if disk9 != nil && (d.HasChange("kibana_configuration.0.disk") || disk9 != "") {
			dataList4["disk"] = disk9
		}
		amount7, _ := jsonpath.Get("$[0].amount", v)
		if amount7 != nil && (d.HasChange("kibana_configuration.0.amount") || amount7 != "") {
			dataList4["amount"] = amount7
		}
		spec9, _ := jsonpath.Get("$[0].spec", v)
		if spec9 != nil && (d.HasChange("kibana_configuration.0.spec") || spec9 != "") {
			dataList4["spec"] = spec9
		}

		request["kibanaConfiguration"] = dataList4
	}
	...
	if v, ok := d.GetOk("pvl_id"); ok {
		query["pvlId"] = StringPointer(v.(string))
	}
