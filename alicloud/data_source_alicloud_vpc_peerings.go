...
func dataSourceAlicloudVpcPeerings() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudVpcPeeringsRead,
...
func dataSourceAlicloudVpcPeeringsRead(d *schema.ResourceData, meta interface{}) error {
	...
	var peeringNameRegex *regexp.Regexp
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_vpc_peerings", action, AlibabaCloudSdkGoERROR)
		}
		...
		for _, v := range result {
			...
			if peeringNameRegex != nil && !peeringNameRegex.MatchString(fmt.Sprint(item["Name"])) {
				continue
			}
			if len(idsMap) > 0 {
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"peering_id":          fmt.Sprint(object["InstanceId"]),
			"peering_name":        object["Name"],
	...
	if err := d.Set("peerings", s); err != nil {
		return WrapError(err)
	}
