...
func resourceAlicloudPolarDBGatewayCostRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err = d.Set("input_cost_points_per_million", object["InputCostPointsPerMillion"]); err != nil {
		return WrapError(err)
	}
	if err = d.Set("output_cost_points_per_million", object["OutputCostPointsPerMillion"]); err != nil {
		return WrapError(err)
	}
	if err = d.Set("cache_cost_points_per_million", object["CacheCostPointsPerMillion"]); err != nil {
		return WrapError(err)
	}
...
func resourceAlicloudPolarDBGatewayCostRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err = callPolarDBGatewayAI(client, "ModifyCostRule", request, d.Timeout(schema.TimeoutUpdate)); err != nil {
