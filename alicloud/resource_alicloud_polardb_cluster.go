				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"endpoint_type": {
							...
							Optional:     true,
							Computed:     true,
							ValidateFunc: StringInSlice([]string{"Primary", "Cluster"}, false),
						},
						"endpoint_config": {
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"pin_once_after_write": {
										Type:     schema.TypeString,
										Computed: true,
										Optional: true,
									},
									"multiplexing_persist_max_time": {
										Type:     schema.TypeString,
										Computed: true,
										Optional: true,
									},
...
func resourceAlicloudPolarDBClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	// endpoint
	if !d.IsNewResource() && d.HasChange("endpoint_system") {
		if err := polarDBService.ModifyDBClusterEndpointInfo(d); err != nil {
			return WrapError(err)
		}
	}
	// ssl
	if !d.IsNewResource() && d.HasChange("ssl_system") {
	...
	if v, ok := d.GetOk("endpoint_system"); ok {
		...
		if len(endpointSystem) >= 1 {
				...
				for _, n := range endpointSystem {
					if "Cluster" == n.(map[string]interface{})["endpoint_type"] {
						rebuildEndpointSystem[1] = map[string]interface{}{
							"db_endpoint_id":          n.(map[string]interface{})["db_endpoint_id"],
			...
			for _, e := range rebuildEndpointSystem {
				dbEndpointIdIndex := e["db_endpoint_id"].(string)
				dbEndpointId := ""
				if len(dbEndpointIdIndex) > 2 {
					dbEndpointId = dbEndpointIdIndex
				} else {
					dbEndpointId = endpoints[formatInt(dbEndpointIdIndex)].DBEndpointId
				}
				newEndpoint := map[string]interface{}{
					"db_endpoint_id_index": dbEndpointIdIndex,
			...
			for _, newEndpoint := range endpointIdList {
				if endpoint.DBEndpointId == newEndpoint["db_endpoint_id"] {
					childEndpoint := map[string]interface{}{
						"db_endpoint_id":          newEndpoint["db_endpoint_id_index"],
	...
	if v, ok := d.GetOk("ssl_system"); ok {
		...
		if len(SSLSystem) >= 1 {
			...
			for _, e := range rebuildSSLSystem {
				...
				newEndpointId := map[string]interface{}{
					"db_endpoint_id_index": dbEndpointIdIndex,
					"db_endpoint_id":       dbEndpointId,
					"net_type":             netType,
				}
				sslEndpointIdList = append(sslEndpointIdList, newEndpointId)
			}
		}
	}
			...
			if len(sslEndpointIdList) > 0 {
				for _, sslNewEndpoint := range sslEndpointIdList {
					if item.DBEndpointId == sslNewEndpoint["db_endpoint_id"] {
						childSSL["db_endpoint_id"] = sslNewEndpoint["db_endpoint_id_index"]
...
func convertPolarDBEndpointConfigSetToMap(source string) []map[string]interface{} {
	...
	if endpointConfig != nil {
		mapping := map[string]interface{}{
			...
			"backed_port":                   endpointConfig["BACKEND_PORT"],
			"master_accept_reads":           endpointConfig["MasterAcceptReads"],
			"connpool_strict_client_ip":     endpointConfig["ConnpoolStrictCLientIp"],
			"enable_htap_imci":              endpointConfig["EnableHtapImci"],
			"pin_prepared_stmt":             endpointConfig["PinPreparedStmt"],
			...
			"strict_sp_calls":               endpointConfig["StrictSPCalls"],
			"causal_consist_read":           endpointConfig["CausalConsistRead"],
			"max_parallel_degree":           endpointConfig["MaxParallelDegree"],
			"track_lsn_per_tab":             endpointConfig["TrackLSNPerTab"],
			"consist_level":                 endpointConfig["ConsistLevel"],
			"enable_sql_template":           endpointConfig["EnableSqlTemplate"],
			"strict_multi_stmt":             endpointConfig["StrictMultiStmt"],
			"trx_break_continue":            endpointConfig["TrxBreakContinue"],
			"multiplexing_persist_pool_max": endpointConfig["MultiplexingPersistPoolMax"],
			"consist_timeout":               endpointConfig["ConsistTimeout"],
			"load_balance_policy":           endpointConfig["LoadBalancePolicy"],
			"pin_once_after_write":          endpointConfig["PinOnceAfterWrite"],
			"multiplexing_persist_max_time": endpointConfig["MultiplexingPersistMaxTime"],
			"polar_cluster_group":           endpointConfig["PolarClusterGroup"],
			"persist_max_time":              endpointConfig["PersistMaxTime"],
			"load_balance_strategy":         endpointConfig["LoadBalanceStrategy"],
			"persist_pool_max":              endpointConfig["PersistPoolMax"],
		}
