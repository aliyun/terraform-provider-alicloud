...
func resourceAlicloudPolarDBClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChanges("scale_min", "scale_max", "allow_shut_down", "scale_ro_num_min", "scale_ro_num_max", "seconds_until_auto_pause", "scale_ap_ro_num_min", "scale_ap_ro_num_max", "serverless_rule_mode", "serverless_rule_cpu_shrink_threshold", "serverless_rule_cpu_enlarge_threshold") {
		...
		serverlessRuleMode := d.Get("serverless_rule_mode")
		if serverlessRuleMode != nil {
			request["ServerlessRuleMode"] = serverlessRuleMode
		}
		serverlessRuleCpuShrinkThreshold := d.Get("serverless_rule_cpu_shrink_threshold")
		if serverlessRuleCpuShrinkThreshold != nil {
			request["ServerlessRuleCpuShrinkThreshold"] = serverlessRuleCpuShrinkThreshold
		}
		serverlessRuleCpuEnlargeThreshold := d.Get("serverless_rule_cpu_enlarge_threshold")
		if serverlessRuleCpuEnlargeThreshold != nil {
			request["ServerlessRuleCpuEnlargeThreshold"] = serverlessRuleCpuEnlargeThreshold
		}
		//retry
		wait := incrementalWait(3*time.Second, 3*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
