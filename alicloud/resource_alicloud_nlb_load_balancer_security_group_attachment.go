...
func resourceAliCloudNlbLoadBalancerSecurityGroupAttachmentCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(fmt.Sprintf("%v:%v", request["LoadBalancerId"], request["SecurityGroupIds[*]"]))
...
func resourceAliCloudNlbLoadBalancerSecurityGroupAttachmentRead(d *schema.ResourceData, meta interface{}) error {
	...
	objectRaw, err := nlbServiceV2.DescribeNlbLoadBalancerSecurityGroupAttachment(d.Id())
	if err != nil {
	...
	securityGroupIds1Raw, _ := jsonpath.Get("$.SecurityGroupIds", objectRaw)
	d.Set("security_group_id", convertNlbSecurityGroupIdsResponse(securityGroupIds1Raw["SecurityGroupIdsChild"]))
