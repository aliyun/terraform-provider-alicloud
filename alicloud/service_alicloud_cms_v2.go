...
// DescribeCmsDataset <<< Encapsulated get interface for Cms Dataset.
func (s *CmsServiceV2) DescribeCmsDataset(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"WorkspaceNotExist", "DatasetNotExist", "WorkspaceNotAvailable"}) {
			return object, WrapErrorf(NotFoundErr("Cms:Dataset", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
// DescribeCmsSubscription <<< Encapsulated get interface for Cms Subscription.

func (s *CmsServiceV2) DescribeCmsSubscription(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"NotFound", "ResourceNotFound"}) {
...
func (s *CmsServiceV2) CmsSubscriptionStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsSubscriptionStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsSubscription)
}

func (s *CmsServiceV2) CmsSubscriptionStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCmsSubscription >>> Encapsulated.

// DescribeCmsNotifyStrategy <<< Encapsulated get interface for Cms NotifyStrategy.

func (s *CmsServiceV2) DescribeCmsNotifyStrategy(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	action := fmt.Sprintf("/notifyStrategies/%s", id)
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"NotFound", "ResourceNotFound"}) {
			return object, WrapErrorf(NotFoundErr("NotifyStrategy", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *CmsServiceV2) CmsNotifyStrategyStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsNotifyStrategyStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsNotifyStrategy)
}

func (s *CmsServiceV2) CmsNotifyStrategyStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCmsNotifyStrategy >>> Encapsulated.
