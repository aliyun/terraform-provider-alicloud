...
func (s *CmsServiceV2) DescribeCmsTransformer(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	request = make(map[string]interface{})
	query = make(map[string]*string)
	query["transformerId"] = StringPointer(parts[0])
	query["workspace"] = StringPointer(parts[1])
	query["regionId"] = StringPointer(client.RegionId)
	action := fmt.Sprintf("/transformers/%s", parts[0])
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"NotFound", "ResourceNotFound"}) {
			return object, WrapErrorf(NotFoundErr("Transformer", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.transformer", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.transformer", response)
	}

	return v.(map[string]interface{}), nil
}

func (s *CmsServiceV2) CmsTransformerStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsTransformerStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsTransformer)
}

func (s *CmsServiceV2) CmsTransformerStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
