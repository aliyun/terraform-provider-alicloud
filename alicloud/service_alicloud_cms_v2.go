...
// DescribeCmsUmodel <<< Encapsulated get interface for Cms Umodel.

func (s *CmsServiceV2) DescribeCmsUmodel(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"UmodelNotExist", "WorkspaceNotAvailable", "WorkspaceNotExist"}) {
			return object, WrapErrorf(NotFoundErr("Cms:Umodel", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *CmsServiceV2) CmsUmodelStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsUmodelStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsUmodel)
}

func (s *CmsServiceV2) CmsUmodelStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCmsUmodel >>> Encapsulated.
