...
func resourceAlicloudEcsDiskCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("availability_zone"); ok {
		_, err := ecsService.DescribeZone(v.(string))
		if err != nil {
			return WrapError(err)
		}
		request["ZoneId"] = v
	...
	if v, ok := d.GetOk("category"); ok {
		...
		if availabilityZone, ok := d.GetOk("availability_zone"); ok {
			zone = availabilityZone.(string)
		} else if instanceId, ok := d.GetOk("instance_id"); ok {
			instance, err := ecsService.DescribeInstance(instanceId.(string))
			if err != nil {
				return WrapError(err)
			}
			zone = instance.ZoneId
		}
		availabilityZone, err := ecsService.DescribeZone(zone)
		if err != nil {
			return WrapError(err)
		}
...
func resourceAlicloudEcsDiskRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("payment_type", convertEcsDiskPaymentTypeResponse(formatECSAlicloudEcsDiskPaymentTypeString(object["DiskChargeType"])))
...
func resourceAlicloudEcsDiskUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if update {
		if _, ok := d.GetOkExists("auto_pay"); ok {
			modifyDiskChargeTypeReq["AutoPay"] = d.Get("auto_pay")
		}
...
func convertEcsDiskPaymentTypeResponse(source string) string {
	switch source {
	case "PostPaid":
		return "PayAsYouGo"
	case "PrePaid":
...
func formatECSAlicloudEcsDiskPaymentTypeString(source interface{}) string {
	if source == nil {
		return ""
	} else {
...
func convertEcsDiskPaymentTypeRequest(source string) string {
	switch source {
	case "PayAsYouGo":
		return "PostPaid"
	case "Subscription":
