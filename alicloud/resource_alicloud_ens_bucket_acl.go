...
// ensBucketAclReadStable polls GetBucketAcl until the value stabilizes. When
// expected is non-empty (the normal Read path after Create/Update, where the
// desired ACL is known from config/state), it requires two consecutive reads
// matching the expected value — the ENS backend oscillates between the old and
// new ACL after a write, so "two consecutive identical reads" alone can latch
// onto the stale old value. When expected is empty (import/refresh with no
// prior state), it falls back to requiring two consecutive identical reads.
// NotFound is not retried here: in import/refresh semantics a missing resource
// should be surfaced so the caller can drop it from state.
func ensBucketAclReadStable(client *connectivity.AliyunClient, id, expected string, timeout time.Duration) (map[string]interface{}, error) {
	ensServiceV2 := EnsServiceV2{client}
	wait := incrementalWait(3*time.Second, 5*time.Second)
	var prevValue string
	consecutive := 0
	...
	err := resource.Retry(timeout, func() *resource.RetryError {
		...
		if expected != "" && current != expected {
			// ENS oscillates between old and new ACL values after a write; two
			// consecutive identical reads can latch onto the stale old value.
			// When the expected value is known, require it before accepting.
			consecutive = 0
			prevValue = ""
			wait()
			return resource.RetryableError(fmt.Errorf("ENS bucket acl %s not converged to expected %s yet (got %s)", id, expected, current))
		}
		if current == prevValue {
			consecutive++
		} else {
...
func resourceAliCloudEnsBucketAclRead(d *schema.ResourceData, meta interface{}) error {
	...
	expected := d.Get("bucket_acl").(string)
	objectRaw, err := ensBucketAclReadStable(client, d.Id(), expected, d.Timeout(schema.TimeoutRead))
	if err != nil {
