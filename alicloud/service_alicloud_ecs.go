...
func (s *EcsService) DescribeEcsImagePipelineExecution(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewEcsClient()
	if err != nil {
		return nil, WrapError(err)
	...
	if len(v.([]interface{})) < 1 {
		...
		if fmt.Sprint(v.([]interface{})[0].(map[string]interface{})["ExecutionId"]) != id {
			return object, WrapErrorf(Error(GetNotFoundMessage("ECS", id)), NotFoundWithResponse, response)
		}
	}
	object = v.([]interface{})[0].(map[string]interface{})
...
func (s *EcsService) EcsImagePipelineExecutionStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
