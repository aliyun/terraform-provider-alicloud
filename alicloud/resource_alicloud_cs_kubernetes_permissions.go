...
func resourceAlicloudCSKubernetesPermissionsCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return resource.RetryableError(Error("[ERROR] Grant user permission failed due to %++v", err))
	})
	if err != nil {
...
func resourceAlicloudCSKubernetesPermissionsUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("permissions") {
		...
		if len(n) == 0 {
			...
			if err != nil {
				return WrapErrorf(err, DefaultErrorMsg, d.Id(), "RemoveSomeClustersPermissions", AlibabaCloudSdkGoERROR)
			}
			d.Partial(false)
			return resourceAlicloudCSKubernetesPermissionsRead(d, meta)
		}
		...
		if err != nil {
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), "UpdateClusterPermissions", AlibabaCloudSdkGoERROR)
		}
		d.Partial(false)
		return resourceAlicloudCSKubernetesPermissionsRead(d, meta)
	}
