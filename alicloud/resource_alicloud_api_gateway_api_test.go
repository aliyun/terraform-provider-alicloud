...
func testAccCheckAlicloudApigatewayApiExists(n string, d *cloudapi.DescribeApiResponse) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		resp, err := cloudApiService.DescribeApi(rs.Primary.ID, rs.Primary.Attributes["group_id"])
		if err != nil {
...
func testAccCheckAlicloudApigatewayApiDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := cloudApiService.DescribeApi(rs.Primary.ID, rs.Primary.Attributes["group_id"])
		if err != nil {
