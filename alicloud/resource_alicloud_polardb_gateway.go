...
func resourceAlicloudPolarDBGateway() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"db_type": {
				Type: schema.TypeString, Optional: true, ForceNew: true,
				ValidateFunc: validation.StringInSlice([]string{"MySQL", "PostgreSQL"}, false),
...
func resourceAlicloudPolarDBGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	for key, requestKey := range map[string]string{
		"zone_id": "ZoneId", "db_cluster_class": "DBClusterClass", "period": "Period",
		"used_time": "UsedTime", "security_group_id": "SecurityGroupId", "db_type": "DBType",
	} {
	...
	stateConf := BuildStateConf([]string{"CREATE"}, []string{"ACTIVATION"}, d.Timeout(schema.TimeoutCreate), 30*time.Second, service.PolarDBGatewayStateRefreshFunc(d.Id(), d.Get("region_id").(string), []string{}))
...
func resourceAlicloudPolarDBGatewayRead(d *schema.ResourceData, meta interface{}) error {
	service := PolarDBService{meta.(*connectivity.AliyunClient)}
	object, err := service.DescribePolarDBGatewayAttributeWithRegion(d.Id(), d.Get("region_id").(string))
	if err != nil {
	...
	for key, responseKey := range map[string]string{
		"region_id": "RegionId", "vpc_id": "VpcId", "vswitch_id": "VSwitchId",
		"pay_type": "PayType", "db_type": "DbType", "db_cluster_class": "ClassCode",
...
func resourceAlicloudPolarDBGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	...
	request := map[string]interface{}{"GwClusterId": d.Id(), "RegionId": d.Get("region_id")}
	var response map[string]interface{}
	var err error
	wait := incrementalWait(3*time.Second, 3*time.Second)
	...
	stateConf := BuildStateConf([]string{"CREATE", "ACTIVATION", "DELETING"}, []string{}, d.Timeout(schema.TimeoutDelete), 10*time.Second, service.PolarDBGatewayStateRefreshFunc(d.Id(), d.Get("region_id").(string), []string{}))
	if _, err = stateConf.WaitForState(); err != nil {
...
func flattenPolarDBGatewayEndpoints(raw interface{}) []map[string]interface{} {
	...
	for _, item := range items {
		result = append(result, map[string]interface{}{
			...
			"port": item["Port"], "tunnel_id": item["TunnelId"], "vpc_id": item["VpcId"], "network_type": item["NetType"],
		})
	}
	return result
}
