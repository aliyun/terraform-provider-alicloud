	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"source": {
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: validation.StringInSlice([]string{"OSS", "Registry", "ExportTask", "Upload"}, false),
			},
			"source_path": {
				Type:     schema.TypeString,
				Optional: true,
...
func resourceAliCloudIacServiceModuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("group_info"); ok {
		groupInfoMap := make(map[string]interface{})
		for _, dataLoop := range convertToInterfaceArray(v) {
			dataLoopTmp := dataLoop.(map[string]interface{})
			if gid, ok := dataLoopTmp["group_id"]; ok && fmt.Sprint(gid) != "" {
				groupInfoMap["groupId"] = gid
			}
			if pid, ok := dataLoopTmp["project_id"]; ok && fmt.Sprint(pid) != "" {
				groupInfoMap["projectId"] = pid
			}
		}
		if len(groupInfoMap) > 0 {
			request["groupInfo"] = groupInfoMap
		}
	}
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_iac_service_module", action, AlibabaCloudSdkGoERROR)
	}
...
func resourceAliCloudIacServiceModuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_iac_service_module DescribeIacServiceModule Failed!!! %s", err)
			d.SetId("")
			return nil
		}
