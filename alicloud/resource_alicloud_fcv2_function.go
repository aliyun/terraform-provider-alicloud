...
func resourceAliCloudFcv2FunctionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("custom_health_check_config"); ok {
		customHealthCheckConfig := make(map[string]interface{})
		nodeNative20, _ := jsonpath.Get("$[0].http_get_url", v)
		if nodeNative20 != "" {
			customHealthCheckConfig["httpGetUrl"] = nodeNative20
		}
		nodeNative21, _ := jsonpath.Get("$[0].initial_delay_seconds", v)
		if nodeNative21 != "" {
			customHealthCheckConfig["initialDelaySeconds"] = nodeNative21
		}
		nodeNative22, _ := jsonpath.Get("$[0].period_seconds", v)
		if nodeNative22 != "" {
			customHealthCheckConfig["periodSeconds"] = nodeNative22
		}
		nodeNative23, _ := jsonpath.Get("$[0].timeout_seconds", v)
		if nodeNative23 != "" {
			customHealthCheckConfig["timeoutSeconds"] = nodeNative23
		}
		nodeNative24, _ := jsonpath.Get("$[0].failure_threshold", v)
		if nodeNative24 != "" {
			customHealthCheckConfig["failureThreshold"] = nodeNative24
		}
		nodeNative25, _ := jsonpath.Get("$[0].success_threshold", v)
		if nodeNative25 != "" {
			customHealthCheckConfig["successThreshold"] = nodeNative25
		}
	...
	if v, ok := d.GetOk("layers"); ok {
		nodeNative29, _ := jsonpath.Get("$", v)
		if nodeNative29 != "" {
			objectDataLocalMap["layers"] = nodeNative29
		}
	}
	if v, ok := d.GetOk("custom_runtime_config"); ok {
		customRuntimeConfig := make(map[string]interface{})
		nodeNative30, _ := jsonpath.Get("$[0].command", v)
		if nodeNative30 != "" {
			customRuntimeConfig["command"] = nodeNative30
		}
		nodeNative31, _ := jsonpath.Get("$[0].args", v)
		if nodeNative31 != "" {
			customRuntimeConfig["args"] = nodeNative31
		}
	...
	if v, ok := d.GetOk("custom_dns"); ok {
		...
		nodeNative34, _ := jsonpath.Get("$[0].name_servers", v)
		if nodeNative34 != "" {
			customDNS["nameServers"] = nodeNative34
		}
		nodeNative35, _ := jsonpath.Get("$[0].searches", v)
		if nodeNative35 != "" {
			customDNS["searches"] = nodeNative35
		}
	...
	d.SetId(fmt.Sprintf("%v:%v", request["serviceName"], response["functionName"]))
...
func resourceAliCloudFcv2FunctionRead(d *schema.ResourceData, meta interface{}) error {
	...
	layers1Raw := make([]interface{}, 0)
	if objectRaw["layers"] != nil {
		layers1Raw = objectRaw["layers"].([]interface{})
	}

	d.Set("layers", layers1Raw)
...
func resourceAliCloudFcv2FunctionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("custom_dns"); ok {
		customDNS := make(map[string]interface{})
		nodeNative29, _ := jsonpath.Get("$[0].name_servers", v)
		if nodeNative29 != "" {
			customDNS["nameServers"] = nodeNative29
		}
		nodeNative30, _ := jsonpath.Get("$[0].name_servers", v)
		if nodeNative30 != "" {
			customDNS["nameServers"] = nodeNative30
		}
		nodeNative31, _ := jsonpath.Get("$[0].searches", v)
		if nodeNative31 != "" {
			customDNS["searches"] = nodeNative31
		}
		nodeNative32, _ := jsonpath.Get("$[0].dns_options", v)
		if nodeNative32 != "" {
			customDNS["dnsOptions"] = nodeNative32
		}
		if v, ok := d.GetOk("custom_dns"); ok {
			localData, err := jsonpath.Get("$[0].dns_options", v)
			if err != nil {
	...
	if v, ok := d.GetOk("custom_runtime_config"); ok {
		customRuntimeConfig := make(map[string]interface{})
		nodeNative36, _ := jsonpath.Get("$[0].command", v)
		if nodeNative36 != "" {
			customRuntimeConfig["command"] = nodeNative36
		}
		nodeNative37, _ := jsonpath.Get("$[0].command", v)
		if nodeNative37 != "" {
			customRuntimeConfig["command"] = nodeNative37
		}
		nodeNative38, _ := jsonpath.Get("$[0].args", v)
		if nodeNative38 != "" {
			customRuntimeConfig["args"] = nodeNative38
		}
		nodeNative39, _ := jsonpath.Get("$[0].args", v)
		if nodeNative39 != "" {
			customRuntimeConfig["args"] = nodeNative39
		}
		objectDataLocalMap["customRuntimeConfig"] = customRuntimeConfig
	}
	if v, ok := d.GetOk("environment_variables"); ok {
