...
func (s *MongoDBService) DescribeMongodbShardingNode(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewDdsClient()
	if err != nil {
		return nil, WrapError(err)
	...
	v, err := jsonpath.Get("$.DBInstances.DBInstance[*].ShardList.ShardAttribute[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.DBInstances.DBInstance[*].ShardList.ShardAttribute[*]", response)
	}
	if len(v.([]interface{})) < 1 {
		return object, WrapErrorf(Error(GetNotFoundMessage("MongoDB", id)), NotFoundWithResponse, response)
	}
	for _, vv := range v.([]interface{}) {
		if fmt.Sprint(vv.(map[string]interface{})["NodeId"]) == parts[1] {
			idExist = true
...
func (s *MongoDBService) MongodbShardingNodeStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeMongodbShardingNode(id)
		if err != nil {
			if NotFoundError(err) {
				// Set this to nil as if we didn't find anything.
				return nil, "", nil
