...
func resourceAlicloudSaeNamespaceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_sae_namespace", "AlicloudSaeNamespaceCreate", AlibabaCloudSdkGoERROR)
	}
	if respBody, isExist := response["body"]; isExist {
		response = respBody.(map[string]interface{})
	} else {
		return WrapError(fmt.Errorf("%s failed, response: %v", "AlicloudSaeNamespaceCreate", response))
	}
	addDebug(action, response, request)
	if fmt.Sprint(response["Success"]) == "false" {
		return WrapError(fmt.Errorf("%s failed, response: %v", "AlicloudSaeNamespaceCreate", response))
	}
	if _, exist := request["NamespaceId"]; !exist {
		return WrapError(fmt.Errorf("%s failed, response: %v", "AlicloudSaeNamespaceCreate", response))
	}
...
func resourceAlicloudSaeNamespaceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if update {
		...
		if err != nil {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), "AlicloudSaeNamespaceUpdate", AlibabaCloudSdkGoERROR)
		}
		if respBody, isExist := response["body"]; isExist {
			response = respBody.(map[string]interface{})
		} else {
			return WrapError(fmt.Errorf("%s failed, response: %v", "AlicloudSaeNamespaceUpdate", response))
		}
		addDebug(action, response, request)
		if fmt.Sprint(response["Success"]) == "false" {
