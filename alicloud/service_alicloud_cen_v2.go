...
// DescribeCenTransitRouterRouteTable <<< Encapsulated get interface for Cen TransitRouterRouteTable.

func (s *CenServiceV2) DescribeCenTransitRouterRouteTable(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	request = make(map[string]interface{})
	query = make(map[string]interface{})
	request["TransitRouterRouteTableIds.1"] = id
	...
	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("TransitRouterRouteTable", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *CenServiceV2) CenTransitRouterRouteTableStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CenTransitRouterRouteTableStateRefreshFuncWithApi(id, field, failStates, s.DescribeCenTransitRouterRouteTable)
}

func (s *CenServiceV2) CenTransitRouterRouteTableStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCenTransitRouterRouteTable >>> Encapsulated.
