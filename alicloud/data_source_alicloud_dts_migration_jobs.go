	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"status": {
				...
				Optional:     true,
				ForceNew:     true,
				ValidateFunc: validation.StringInSlice([]string{"Downgrade", "Finished", "Locked", "Migrating", "MigrationFailed", "NotConfigured", "NotStarted", "PreCheckPass", "PrecheckFailed", "Prechecking", "Retrying", "Suspending", "Upgrade", "Failed"}, true),
			},
			"names": {
				Type:     schema.TypeList,
				Elem:     &schema.Schema{Type: schema.TypeString},
