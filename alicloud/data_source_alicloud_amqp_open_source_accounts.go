...
func dataSourceAliCloudAmqpOpenSourceAccountRead(d *schema.ResourceData, meta interface{}) error {
	...
	for _, objectRaw := range objects {
		...
		id := fmt.Sprint(objectRaw["Name"], ":", objectRaw["CInstanceId"])
		mapping, err = dataSourceAliCloudAmqpOpenSourceAccountReadDescription(d, id, mapping, meta)
		if err != nil {
...
func dataSourceAliCloudAmqpOpenSourceAccountReadDescription(d *schema.ResourceData, id string, object map[string]interface{}, meta interface{}) (map[string]interface{}, error) {
	client := meta.(*connectivity.AliyunClient)

	amqpServiceV2 := AmqpServiceV2{client}
	getResp, err := amqpServiceV2.DescribeAmqpOpenSourceAccount(id)
	if err != nil {
