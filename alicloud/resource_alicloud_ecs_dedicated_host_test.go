...
func TestAccAliCloudECSDedicatedHost_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Change payment_type from PrePaid to PostPaid to exercise the
			// ModifyDedicatedHostsChargeType update path. period and
			// period_unit are carried over from the create step so the
			// renewal branch (HasChange period) is not re-triggered; only
