...
// DescribeEnsNetworkPeerConnection <<< Encapsulated get interface for Ens NetworkPeerConnection.

func (s *EnsServiceV2) DescribeEnsNetworkPeerConnection(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	v, err := jsonpath.Get("$.NetworkPeerConnects[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.NetworkPeerConnects[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("NetworkPeerConnection", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *EnsServiceV2) EnsNetworkPeerConnectionStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeEnsNetworkPeerConnection(id)
		if err != nil {
			if NotFoundError(err) {
				return nil, "", nil
			}
...
// DescribeEnsNetworkPeerConnection >>> Encapsulated.
