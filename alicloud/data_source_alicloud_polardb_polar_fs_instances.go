...
func dataSourceAlicloudPolarDBPolarFsInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	request := map[string]interface{}{
		"RegionId": client.RegionId, "PageSize": 100, "PageNumber": 1,
		// DescribePolarFs defaults to querying non-OSS backends when PolarFsType
		// is omitted. A data source without a type filter must return all types.
		"PolarFsType": "all",
	}
	...
	for _, value := range all {
		...
		storageSpace, err := normalizePolarDBPolarFsInteger(item["StorageSpace"])
		if err != nil {
			return WrapErrorf(err, "failed to decode StorageSpace from %v", item["StorageSpace"])
		}
		bandwidth, err := normalizePolarDBPolarFsInteger(item["Bandwidth"])
		if err != nil {
			return WrapErrorf(err, "failed to decode Bandwidth from %v", item["Bandwidth"])
		}
		ids = append(ids, id)
		instances = append(instances, map[string]interface{}{
			...
			"zone_id": item["ZoneId"], "storage_space": storageSpace, "storage_type": item["StorageType"],
			"expired": item["Expired"], "bandwidth": bandwidth, "polar_fs_type": item["PolarFsType"],
