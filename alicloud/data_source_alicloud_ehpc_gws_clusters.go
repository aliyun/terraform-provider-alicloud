...
func dataSourceAlicloudEhpcGwsClusters() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudEhpcGwsClustersRead,
		Schema: map[string]*schema.Schema{
			...
			"status": {
				...
				Optional:     true,
				ForceNew:     true,
				ValidateFunc: validation.StringInSlice([]string{"creating", "deleted", "running", "starting"}, false),
			},
			"output_file": {
				Type:     schema.TypeString,
				Optional: true,
...
func dataSourceAlicloudEhpcGwsClustersRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_ehpc_gws_clusters", action, AlibabaCloudSdkGoERROR)
		}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"gws_cluster_id": fmt.Sprint(object["ClusterId"]),
