...
func testSweepVpcFlowLog(region string) error {
	...
	for {
		...
		if err != nil {
			log.Printf("[ERROR] Failed to retrieving Vpc flow log: %s", err)
			break
		}
	...
	for _, v := range insts {
		...
		if skip {
			log.Printf("[INFO] Skipping Vpc flow log: %s (%s)", name, id)
			continue
		}
		sweeped = true
		...
		if err != nil {
			log.Printf("[ERROR] Failed to delete Vpc flow log (%s (%s)): %s", name, id, err)
		}
	}
	if sweeped {
...
// One Vpc Instance only can have one flow log.
