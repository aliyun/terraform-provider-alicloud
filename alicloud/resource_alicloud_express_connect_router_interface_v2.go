...
func resourceAliCloudExpressConnectRouterInterfaceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOkExists("opposite_interface_owner_id"); ok {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_express_connect_router_interface_v2", action, AlibabaCloudSdkGoERROR)
	}
...
func resourceAliCloudExpressConnectRouterInterfaceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_express_connect_router_interface_v2 DescribeExpressConnectRouterInterface Failed!!! %s", err)
			d.SetId("")
			return nil
		}
	...
	d.Set("hc_threshold", objectRaw["HcThreshold"])
	...
	d.Set("opposite_interface_owner_id", formatInt(objectRaw["OppositeInterfaceOwnerId"]))
...
func resourceAliCloudExpressConnectRouterInterfaceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if enableDelete {
		...
		if v, ok := d.GetOkExists("payment_type"); ok {
			if InArray(fmt.Sprint(v), []string{"PostPaid"}) {
				enableDelete = true
			}
		}
		if enableDelete {
	...
	if enableDelete {
		enableDelete = false
		if v, ok := d.GetOkExists("payment_type"); ok {
			if InArray(fmt.Sprint(v), []string{"PrePaid"}) {
				enableDelete = true
			}
		}
		if enableDelete {
