...
func dataSourceAlicloudMseEngineConfigs() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudMseEngineConfigsRead,
...
func dataSourceAlicloudMseEngineConfigsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_mse_engine_configs", action, AlibabaCloudSdkGoERROR)
		}
	...
	for _, object := range objects {
		...
		getResp, err := mseService.DescribeMseEngineConfig(id)
		if err != nil {
			return WrapError(err)
		}
