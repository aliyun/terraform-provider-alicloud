...
func TestAccAliCloudPolarDBZonalCluster_DBNodeConfig_MYSQL(t *testing.T) {
	var v map[string]interface{}
	name := "tf-testAccPolarDBClusterDBNodeConfigMYSQL"
	resourceId := "alicloud_polardb_on_ens_cluster.default"
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"db_cluster_nodes_configs": map[string]string{
						"nodeWriter": preparedNodesEscape["nodeWriter"],
					},
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"db_cluster_nodes_configs.nodeWriter": preparedNodes["nodeWriter"],
					}),
				),
			},
			{
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"db_cluster_nodes_configs": map[string]string{
						"nodeWriter": preparedNodesEscape["nodeWriterDemote"],
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"db_cluster_nodes_configs.%":          "2",
						"db_cluster_nodes_attributes.%":       "2",
						"db_cluster_nodes_configs.nodeWriter": preparedNodes["nodeWriterDemote"],
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"db_cluster_nodes_configs.%":          "1",
						"db_cluster_nodes_attributes.%":       "1",
						"db_cluster_nodes_configs.nodeWriter": REMOVEKEY,
						"db_cluster_nodes_configs.node1":      preparedNodes["node1Upgrade"],
					}),
				),
				PreventDiskCleanup: true,
...
func prepareNodeConfigs() (map[string]string, map[string]string) {
	nodes := map[string]map[string]string{
		"nodeWriter":       {"db_node_class": "polar.mysql.x4.medium.c", "db_node_role": "Writer"},
