...
func resourceAlicloudLogtailToMachineGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("resourceAlicloudLogtailToMachineGroupRead got an error: %#v.", err)
	}
...
func resourceAlicloudLogtailToMachineGroupDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(3*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			return resource.NonRetryableError(fmt.Errorf("Deleting logtail config from machine group %s got an error: %#v", split[2], err))
		}
		...
		return resource.RetryableError(fmt.Errorf("Deleting logtail config from machine group %s timeout.", split[2]))
	})
}
