...
func resourceAlicloudDtsMigrationJobRead(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := object["DestinationEndpoint"].(map[string]interface{}); ok {
		d.Set("destination_endpoint_data_base_name", v["DatabaseName"])
	...
	if d.HasChange("status") {
		...
		if object["Status"].(string) != target {
			if target == "Migrating" {
				...
				stateConf := BuildStateConf([]string{}, []string{"Migrating"}, d.Timeout(schema.TimeoutUpdate), 5*time.Second, dtsService.DtsMigrationJobStateRefreshFunc(d.Id(), []string{}))
				if _, err := stateConf.WaitForState(); err != nil {
					return WrapErrorf(err, IdMsg, d.Id())
				}
			}
			if target == "Suspending" {
				...
				stateConf := BuildStateConf([]string{}, []string{"Suspending"}, d.Timeout(schema.TimeoutUpdate), 5*time.Second, dtsService.DtsMigrationJobStateRefreshFunc(d.Id(), []string{}))
				if _, err := stateConf.WaitForState(); err != nil {
					return WrapErrorf(err, IdMsg, d.Id())
				}
			}
