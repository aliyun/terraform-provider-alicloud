		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"encrypt_type":                 "HBR_PRIVATE",
						"vault_name":                   CHECKSET,
						"replication_source_region_id": "cn-hangzhou",
						"replication_source_vault_id":  CHECKSET,
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"provider"},
			},
		},
	})
}
...
// Case 全生命周期_全属性_更新对接 2520
func TestAccAliCloudHbrReplicationVault_basic2520(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_hbr_replication_vault.default"
	ra := resourceAttrInit(resourceId, AlicloudHbrReplicationVaultMap2520)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &HbrServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeHbrReplicationVault")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacchbr%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudHbrReplicationVaultBasicDependence2520)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheckWithRegions(t, true, []connectivity.Region{"cn-hangzhou"})
			testAccPreCheck(t)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"description":                  "RDK测试",
					"vault_name":                   "RDK_TEST_backup3",
					"replication_source_region_id": "cn-beijing",
					"replication_source_vault_id":  "v-00022ziygjkqi06cu6b7",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"description":                  "RDK测试",
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"description": "RDK描述更新",
					"vault_name":  "RDK_test_chenyi_update",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"description": "RDK描述更新",
...
var AlicloudHbrReplicationVaultMap2520 = map[string]string{
	"status":              CHECKSET,
	"vault_storage_class": CHECKSET,
	"region_id":           CHECKSET,
}

func AlicloudHbrReplicationVaultBasicDependence2520(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
