...
func resourceAliyunBgpPeerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := bgpPeerService.WaitForBgpPeer(d.Id(), Available, DefaultLongTimeout); err != nil {
...
func resourceAliyunBgpPeerDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	bgpPeerService := BgpService{client}
	if err := bgpPeerService.WaitForBgpPeer(d.Id(), Available, 2*DefaultLongTimeout); err != nil {
		return WrapError(err)
	}
	...
	return WrapError(bgpPeerService.WaitForBgpPeer(d.Id(), Deleted, DefaultLongTimeout))
}
