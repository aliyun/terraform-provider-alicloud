										...
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												...
												"api_version": {
													Type:     schema.TypeString,
													Optional: true,
													// The server does not return apiVersion on readback;
													// suppress the resulting perpetual diff for this
													// deprecated, write-only passthrough field.
...
func resourceAliCloudCmsDigitalEmployeeRead(d *schema.ResourceData, meta interface{}) error {
	...
	if attributes, ok := object["attributes"].(map[string]interface{}); ok {
		...
		for k, vv := range attributes {
			// findingsEnabled is injected by the server and cannot be set by users;
			// reading it back would cause a perpetual diff.
			if k == "findingsEnabled" {
				continue
			}
...
func flattenCmsDigitalEmployeeTags(tags []interface{}) []map[string]interface{} {
	...
	for _, raw := range tags {
		if m, ok := raw.(map[string]interface{}); ok {
			// Skip system tags injected by the platform (e.g. acs:rm:rgId);
			// they are not user-managed and would cause a perpetual diff.
			if key, _ := m["key"].(string); strings.HasPrefix(key, "acs:") {
				continue
			}
			item := map[string]interface{}{
				"key":   m["key"],
...
func flattenCmsDigitalEmployeeKnowledges(knowledges map[string]interface{}) []map[string]interface{} {
	...
	// The server returns a default empty knowledges object (e.g. {"bailian": [],
	// "sop": []}) even when knowledges was never configured; do not surface a
	// phantom block or plans will never be empty.
	if len(item) == 0 {
		return out
	}
	if bailian, ok := item["bailian"].([]map[string]interface{}); ok && len(bailian) == 0 {
		return out
	}
	out = append(out, item)
	return out
