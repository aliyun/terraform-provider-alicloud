...
func resourceAliCloudOssBucketWormCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		response, err = client.RoaPostV2("Oss", "2019-05-17", "InitiateBucketWorm", action, query, body, nil, hostMap, false)
		if err != nil {
			if NeedRetry(err) {
				wait()
...
func resourceAliCloudOssBucketWormUpdate(d *schema.ResourceData, meta interface{}) error {
		...
		if object["State"].(string) != target {
			if target == "Locked" {
				...
				err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
					response, err = client.RoaPostV2("Oss", "2019-05-17", "CompleteBucketWorm", action, query, body, nil, hostMap, false)
					if err != nil {
						if NeedRetry(err) {
							wait()
							return resource.RetryableError(err)
	...
	if update {
		wait := incrementalWait(3*time.Second, 5*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
			response, err = client.RoaPostV2("Oss", "2019-05-17", "ExtendBucketWorm", action, query, body, nil, hostMap, false)
			if err != nil {
				if NeedRetry(err) {
					wait()
					return resource.RetryableError(err)
...
func resourceAliCloudOssBucketWormDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = resource.Retry(d.Timeout(schema.TimeoutDelete), func() *resource.RetryError {
		response, err = client.RoaDeleteV2("Oss", "2019-05-17", "AbortBucketWorm", action, query, nil, nil, hostMap, false)
		if err != nil {
			if NeedRetry(err) {
				wait()
				return resource.RetryableError(err)
