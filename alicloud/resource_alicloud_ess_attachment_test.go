...
func TestAccAliCloudEssAttachment_update(t *testing.T) {
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourceEssAttachmentConfigDependence)
...
func resourceEssAttachmentConfigDependence(name string) string {
	return fmt.Sprintf(`
	%s
	variable "name" {
		default = "%s"
