...
func resourceAlicloudSelectDBDbInstance() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"desired_engine_version": {
				Type:         schema.TypeString,
				ValidateFunc: StringInSlice([]string{string("3.0"), string("4.0")}, false),
				Required:     true,
			},
			"zone_id": {
				Type:     schema.TypeString,
			...
			"maintain_start_time": {
				Type:     schema.TypeString,
				Optional: true,
				Default:  "1970-01-01T18:00Z",
			},
			"maintain_end_time": {
				Type:     schema.TypeString,
				Optional: true,
				Default:  "1970-01-01T22:00Z",
			},
...
func resourceAlicloudSelectDBDbInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	stateConfPreparing := BuildStateConf([]string{"RESOURCE_PREPARING", "CREATING"}, []string{"ACTIVE"}, d.Timeout(schema.TimeoutUpdate), 1*time.Minute, selectDBService.SelectDBDbInstanceStateRefreshFunc(d.Id(), []string{"DELETING"}))
...
func resourceAlicloudSelectDBDbInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && (d.HasChange("db_instance_class")) {
		...
		cache_size := 0
		if d.HasChange("desired_cache_size") {
			_, newCacheSize := d.GetChange("desired_cache_size")
		...
		if d.HasChange("desired_cache_size") {
			d.SetPartial("desired_cache_size")
		}
	}

	if !d.IsNewResource() && d.HasChange("desired_cache_size") && !cacheSizeModified {
		defaultBeId := fmt.Sprint(d.Id() + ":" + d.Id() + "-be")
		_, newCacheSize := d.GetChange("desired_cache_size")
	...
	stateConf := BuildStateConf([]string{""}, []string{"ACTIVE"}, d.Timeout(schema.TimeoutUpdate), 10*time.Second, selectDBService.SelectDBDbInstanceStateRefreshFunc(d.Id(), []string{"DELETING"}))
	if _, err := stateConf.WaitForState(); err != nil {
		return WrapErrorf(err, IdMsg, d.Id())
	}
...
func resourceAlicloudSelectDBDbInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	default_cache_size := d.Get("desired_cache_size").(int)
	...
	d.Set("default_cache_size", default_cache_size)
...
func buildSelectDBCreateInstanceRequest(d *schema.ResourceData, meta interface{}) (map[string]interface{}, error) {
	...
	request := map[string]interface{}{
		"Engine":                "SelectDB",
		"EngineVersion":         Trim(d.Get("desired_engine_version").(string)),
	...
	cache_size, exist := d.GetOkExists("desired_cache_size")
	if exist {
