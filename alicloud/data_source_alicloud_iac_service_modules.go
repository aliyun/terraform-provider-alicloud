...
func dataSourceAliCloudIacServiceModuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	// The ListModules API silently ignores the groupId query parameter, so
	// group_id is filtered client-side below instead of being sent to the server.
	groupId := ""
	...
	for {
		...
		for _, v := range result {
			...
			if groupId != "" {
				groupInfo, _ := item["groupInfo"].(map[string]interface{})
				if fmt.Sprint(groupInfo["groupId"]) != groupId {
					continue
				}
			}
