...
// expandAgentloopPipelineNodeParameters parses string values that carry JSON
// documents into their structured form. The API declares node parameters as
// Map<String, ?> and several node types take arrays/objects/numbers/booleans,
// which a TypeMap of strings cannot represent otherwise (e.g. `"filters":
// "[\"a\",\"b\"]"` must reach the backend as a real array). Values that are
// not valid JSON are sent verbatim.
func expandAgentloopPipelineNodeParameters(raw map[string]interface{}) map[string]interface{} {
	...
	for k, v := range raw {
		...
		// range reach the API unrounded (json.Number marshals back verbatim).
	...
	if nodesRaw, ok := rawMap["nodes"].([]interface{}); ok {
		for _, nodeRaw := range nodesRaw {
			...
			if v, ok := nodeMap["parameters"].(map[string]interface{}); ok && len(v) > 0 {
				node["parameters"] = expandAgentloopPipelineNodeParameters(v)
			}
...
// flattenAgentloopPipelineNodeParameters converts the parameters returned by
// the API back into the TypeMap-of-strings shape: non-string values are
// serialized to their JSON text. When the serialized form is semantically
// equal to the text already held in the state (the user-supplied value), the
// state text is kept verbatim so this ForceNew attribute stays diff-free even
// when the backend reorders keys or normalizes the document.
func flattenAgentloopPipelineNodeParameters(raw interface{}, configured map[string]interface{}) map[string]interface{} {
	result := make(map[string]interface{})
	rawMap, ok := raw.(map[string]interface{})
	if !ok {
		return result
...
func flattenAgentloopPipelinePipeline(raw interface{}, configuredRaw interface{}) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)
	rawMap, ok := raw.(map[string]interface{})
	if !ok || len(rawMap) == 0 {
		return result
	}
	configuredNodes := make([]interface{}, 0)
	if configuredList, ok := configuredRaw.([]interface{}); ok && len(configuredList) > 0 && configuredList[0] != nil {
		if configuredMap, ok := configuredList[0].(map[string]interface{}); ok {
			if nodes, ok := configuredMap["nodes"].([]interface{}); ok {
	...
	if nodesRaw, ok := rawMap["nodes"].([]interface{}); ok {
		for i, nodeRaw := range nodesRaw {
			...
			if i < len(configuredNodes) {
				if configuredNodeMap, ok := configuredNodes[i].(map[string]interface{}); ok {
					configuredParams, _ = configuredNodeMap["parameters"].(map[string]interface{})
				}
			}
			nodeMap := make(map[string]interface{})
			nodeMap["id"] = nodeMapRaw["id"]
			nodeMap["type"] = nodeMapRaw["type"]
			nodeMap["parameters"] = flattenAgentloopPipelineNodeParameters(nodeMapRaw["parameters"], configuredParams)
...
func resourceAliCloudAgentloopPipelineRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("pipeline", flattenAgentloopPipelinePipeline(objectRaw["pipeline"], d.Get("pipeline"))); err != nil {
		return err
	}
