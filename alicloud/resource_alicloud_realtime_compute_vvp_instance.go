...
//
// UNVERIFIED — pending empirical confirmation against a real Flink/VVP
// subscription instance. bss QueryProductList does not list VVP, and the test
// account has no live VVP subscription instance, so QueryAvailableInstances
// could not reveal the billing ProductType (the API accepts any ProductCode and
// returns an empty InstanceList when no instance matches, so absence is not
// proof of an invalid code). ProductCode below is the foasconsole POP product
// code — the only factual identifier available; ProductType is intentionally
// left empty rather than fabricated. SetRenewal treats ProductType as optional;
// if a real instance shows a non-empty ProductType, fill the domestic/intl
// constants below and remove this notice.
const (
	realtimeComputeVvpBssProductCode     = "foasconsole"
	realtimeComputeVvpBssProductType     = ""
	realtimeComputeVvpBssProductTypeIntl = ""
)
...
func setRealtimeComputeVvpInstanceRenewal(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if _, ok := d.GetOkExists("auto_renew_duration"); ok {
			needUpdate = true
		}
