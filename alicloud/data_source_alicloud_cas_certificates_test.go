...
var key_content = `-----BEGIN RSA PRIVATE KEY-----
...
func TestAccAlicloudCasCertificatesDataSource_certificates(t *testing.T) {
	ioutil.WriteFile("./tf_testAcc_cert.crt", []byte(cert_content), 0644)
	ioutil.WriteFile("./tf_testAcc_key.key", []byte(key_content), 0644)
