	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"security_policy_name": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: StringMatch(regexp.MustCompile(`^[a-zA-Z0-9-._]{1,200}$`), "The name must be 1 to 200 characters in length, and can contain letters, digits, periods (.), underscores (_), and hyphens (-)."),
				Computed:     true,
			},
			"status": {
				Type:     schema.TypeString,
...
func resourceAliCloudNlbSecurityPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	nlbServiceV2 := NlbServiceV2{client}
	stateConf := BuildStateConf([]string{}, []string{""}, d.Timeout(schema.TimeoutDelete), 5*time.Second, nlbServiceV2.NlbSecurityPolicyStateRefreshFunc(d.Id(), "SecurityPolicyStatus", []string{}))
	if _, err := stateConf.WaitForState(); err != nil {
		return WrapErrorf(err, IdMsg, d.Id())
	}
	return nil
