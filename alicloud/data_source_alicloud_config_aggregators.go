...
func dataSourceAlicloudConfigAggregatorsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		for _, v := range result {
			...
			if statusOk && status.(int) != formatInt(item["AggregatorStatus"]) {
				continue
			}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"status":          formatInt(object["AggregatorStatus"]),
		}
		if detailedEnabled := d.Get("enable_details"); !detailedEnabled.(bool) {
			ids = append(ids, fmt.Sprint(object["AggregatorId"]))
		...
		if v, ok := getResp["AggregatorAccounts"].([]interface{}); ok {
