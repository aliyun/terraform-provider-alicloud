...
func resourceAliCloudOssBucketMetaQueryCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = resource.Retry(d.Timeout(schema.TimeoutCreate), func() *resource.RetryError {
		response, err = client.Do("Oss", genXmlParam("POST", "2019-05-17", "OpenMetaQuery", action), query, body, nil, hostMap, false)
		if err != nil {
			if NeedRetry(err) {
				wait()
...
func resourceAliCloudOssBucketMetaQueryDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = resource.Retry(d.Timeout(schema.TimeoutDelete), func() *resource.RetryError {
		response, err = client.Do("Oss", genXmlParam("POST", "2019-05-17", "CloseMetaQuery", action), query, body, nil, hostMap, false)
		if err != nil {
			if NeedRetry(err) {
				wait()
				return resource.RetryableError(err)
