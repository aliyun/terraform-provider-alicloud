...
func resourceAlicloudDBCloneInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceAlicloudDBCloneInstanceCreate,
		Read:   resourceAlicloudDBCloneInstanceRead,
		Update: resourceAlicloudDBCloneInstanceUpdate,
		Delete: resourceAlicloudDBCloneInstanceDelete,
...
func resourceAlicloudDBCloneInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	request, err := buildDBCloneCreateRequest(d, meta)
	if err != nil {
		return WrapError(err)
	}
	...
	return resourceAlicloudDBCloneInstanceUpdate(d, meta)
}
...
func resourceAlicloudDBCloneInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		d.Partial(false)
		return resourceAlicloudDBCloneInstanceRead(d, meta)
	}
	...
	if d.HasChange("clone_db_instance") {
		action := "CloneDBInstance"
		request := map[string]interface{}{
			"RegionId":     client.RegionId,
			"DBInstanceId": d.Id(),
		...
		pack := d.Get("clone_db_instance").(map[string]interface{})
		if pack["db_instance_storage_type"] != nil && pack["db_instance_storage_type"] != "" {
			request["DBInstanceStorageType"] = pack["db_instance_storage_type"].(string)
		}
		if pack["instance_charge_type"] != nil && pack["instance_charge_type"] != "" {
			request["PayType"] = pack["instance_charge_type"].(string)
		}
		if pack["zone_id"] != nil && pack["zone_id"] != "" {
			request["ZoneId"] = pack["zone_id"].(string)
		}
		if pack["instance_type"] != nil && pack["instance_type"] != "" {
			request["DBInstanceClass"] = pack["instance_type"].(string)
		}
		if pack["instance_storage"] != nil && pack["instance_storage"] != "" {
			request["DBInstanceStorage"] = pack["instance_storage"].(string)
		}
		if pack["db_names"] != nil && pack["db_names"] != "" {
			request["DbNames"] = pack["db_names"].(string)
		}
		if pack["backup_id"] != nil && pack["backup_id"] != "" {
			request["BackupId"] = pack["backup_id"].(string)
		}
		if pack["restore_time"] != nil && pack["restore_time"] != "" {
			request["RestoreTime"] = pack["restore_time"].(string)
		}
		if pack["vswitch_id"] != nil && pack["vswitch_id"] != "" {
			request["VSwitchId"] = pack["vswitch_id"].(string)
		}
		if pack["private_ip_address"] != nil && pack["private_ip_address"] != "" {
			request["PrivateIpAddress"] = pack["private_ip_address"].(string)
		}
		if pack["period"] != nil && pack["period"] != "" {
			request["Period"] = pack["period"].(string)
		}
		if pack["restore_table"] != nil && pack["restore_table"] != "" {
			request["RestoreTable"] = pack["restore_table"].(string)
		}
		if pack["table_meta"] != nil && pack["table_meta"] != "" {
			request["TableMeta"] = pack["table_meta"].(string)
		}
		if pack["dedicated_host_group_id"] != nil && pack["dedicated_host_group_id"] != "" {
			request["DedicatedHostGroupId"] = pack["dedicated_host_group_id"].(string)
		}
		if pack["backup_type"] != nil && pack["backup_type"] != "" {
			request["BackupType"] = pack["backup_type"].(string)
		}
		var response map[string]interface{}
		wait := incrementalWait(3*time.Second, 3*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
		...
		stateConf := BuildStateConf([]string{}, []string{"Running"}, d.Timeout(schema.TimeoutUpdate), 3*time.Minute, rdsService.RdsDBInstanceStateRefreshFunc(dbInstanceId, []string{"Deleting"}))
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapErrorf(err, IdMsg, d.Id())
		}
		d.SetPartial("clone_db_instance")
		// wait instance status is running after modifying
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapErrorf(err, IdMsg, d.Id())
		}
...
func resourceAlicloudDBCloneInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	rdsService := RdsService{client}
...
func resourceAlicloudDBCloneInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	rdsService := RdsService{client}
...
func buildDBCloneCreateRequest(d *schema.ResourceData, meta interface{}) (map[string]interface{}, error) {
	client := meta.(*connectivity.AliyunClient)
	vpcService := VpcService{client}
	request := map[string]interface{}{
		"RegionId":              client.RegionId,
	...
	if v, ok := d.GetOk("db_names"); ok && v.(string) != "" {
		request["DbNames"] = v
	}
	if v, ok := d.GetOk("backup_id"); ok && v.(string) != "" {
		request["BackupId"] = v
	}
	if v, ok := d.GetOk("restore_time"); ok && v.(string) != "" {
		request["RestoreTime"] = v
	}
	if v, ok := d.GetOk("restore_table"); ok && v.(string) != "" {
		request["RestoreTable"] = v
	}
	if v, ok := d.GetOk("table_meta"); ok && v.(string) != "" {
		request["TableMeta"] = v
	}
	if v, ok := d.GetOk("dedicated_host_group_id"); ok && v.(string) != "" {
		request["DedicatedHostGroupId"] = v
	}

	if v, ok := d.GetOk("backup_type"); ok && v.(string) != "" {
