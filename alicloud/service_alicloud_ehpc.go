...
func (s *EhpcService) DescribeEhpcGwsCluster(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	...
	v, err := jsonpath.Get("$.Clusters.ClusterInfo", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.Clusters.ClusterInfo", response)
	}
	if len(v.([]interface{})) < 1 {
		return object, WrapErrorf(Error(GetNotFoundMessage("Ehpc", id)), NotFoundWithResponse, response)
	} else {
		if fmt.Sprint(v.([]interface{})[0].(map[string]interface{})["ClusterId"]) != id {
			return object, WrapErrorf(Error(GetNotFoundMessage("Ehpc", id)), NotFoundWithResponse, response)
		}
	}
...
func (s *EhpcService) EhpcGwsClusterStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeEhpcGwsCluster(id)
		if err != nil {
