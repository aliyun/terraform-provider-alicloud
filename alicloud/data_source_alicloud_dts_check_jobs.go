...
func dataSourceAlicloudDtsCheckJobs() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudDtsCheckJobsRead,
...
func dataSourceAlicloudDtsCheckJobsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_dts_check_jobs", action, AlibabaCloudSdkGoERROR)
		}
	...
	for _, object := range objects {
		migrationModeObj, _ := object["MigrationMode"].(map[string]interface{})
		destinationEndpointObj, _ := object["DestinationEndpoint"].(map[string]interface{})
		sourceEndpointObj, _ := object["SourceEndpoint"].(map[string]interface{})
		...
		getResp, err := dtsService.DescribeDtsCheckJob(id)
		if err != nil {
			return WrapError(err)
		}
		if src, ok := getResp["SourceEndpoint"].(map[string]interface{}); ok {
			mapping["source_endpoint_owner_id"] = src["AliyunUid"]
			mapping["source_endpoint_role"] = src["RoleName"]
		}
