...
func (s *HBaseService) SetResourceTags(d *schema.ResourceData) error {
...
func (s *HBaseService) DescribeHbaseMultiZoneCluster(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewHbaseClient()
	if err != nil {
		return nil, WrapError(err)
...
func (s *HBaseService) HbaseMultiZoneClusterStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeHbaseMultiZoneCluster(id)
		if err != nil {
			if NotFoundError(err) {
				// Set this to nil as if we didn't find anything.
				return nil, "", nil
