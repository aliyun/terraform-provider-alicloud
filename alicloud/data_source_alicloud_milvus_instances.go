...
func dataSourceAliCloudMilvusInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("tags"); ok {
		...
		tagJSON, _ := json.Marshal(tagList)
		query["tag"] = StringPointer(string(tagJSON))
	}
	...
	for {
		...
		for _, v := range result {
			...
			if instanceNameFilter, ok := d.GetOk("instance_name"); ok {
				if fmt.Sprint(item["instanceName"]) != instanceNameFilter.(string) {
					continue
				}
			}
			if len(idsMap) > 0 {
