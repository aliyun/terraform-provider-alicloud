...
func TestAccAlicloudOssBucketObjectsDataSource_versioning(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckAlicloudOssBucketObjectsDataSourceBasicVersion(randInt),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAlicloudDataSourceID("data.alicloud_oss_bucket_objects.objects"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.#", "2"),
					...
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.0.storage_class", "Standard"),
					resource.TestCheckResourceAttrSet("data.alicloud_oss_bucket_objects.objects", "objects.0.last_modification_time"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.key", fmt.Sprintf("tf-sample/tf-testacc-bucket-object-ds-version-%d-object", randInt)),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.acl", "default"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.content_type", "text/plain"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.content_length", "14"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.cache_control", "max-age=0"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.content_disposition", "attachment; filename=\"my-object\""),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.content_encoding", "gzip"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.content_md5", "1STMBJqp4X5QEQsYTbRmkQ=="),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.expires", "Wed, 06 May 2020 00:00:00 GMT"),
					resource.TestCheckResourceAttrSet("data.alicloud_oss_bucket_objects.objects", "objects.1.etag"),
					resource.TestCheckResourceAttr("data.alicloud_oss_bucket_objects.objects", "objects.1.storage_class", "Standard"),
					resource.TestCheckResourceAttrSet("data.alicloud_oss_bucket_objects.objects", "objects.1.last_modification_time"),
				),
			},
		},
	})
...
func testAccCheckAlicloudOssBucketObjectsDataSourceBasicVersion(randInt int) string {
	return fmt.Sprintf(`
variable "name" {
...
resource "alicloud_oss_bucket_object" "sample_object_2" {
	bucket = "${alicloud_oss_bucket.sample_bucket.bucket}"
	key = "tf-sample/${var.name}-object"
	content = "sample content"
	content_type = "text/plain"
