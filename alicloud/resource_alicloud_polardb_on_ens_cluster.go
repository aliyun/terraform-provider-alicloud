...
func resourceAliCloudPolarDbOnENSCluster() *schema.Resource {
	return &schema.Resource{
		Create: resourceAliCloudPolarDbOnENSClusterCreate,
		Read:   resourceAliCloudPolarDbOnENSClusterRead,
		Update: resourceAliCloudPolarDbOnENSClusterUpdate,
		Delete: resourceAliCloudPolarDbOnENSClusterDelete,
		...
		CustomizeDiff: func(d *schema.ResourceDiff, v interface{}) error {
			if d.HasChange("db_cluster_nodes_configs") {
				...
				if flagNodeCountUpdate {
					err := d.SetNewComputed("db_node_num")
					if err != nil {
						return err
					}
				}
...
func resourceAliCloudPolarDbOnENSClusterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceAliCloudPolarDbOnENSClusterUpdate(d, meta)
}

func resourceAliCloudPolarDbOnENSClusterRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	polarDbServiceV2 := PolarDbServiceV2{client}
	...
	if err = mergeNodeAttributeFromThreeWay(d, &nodeAttributes, &nodeConfigs, objectObj); err != nil {
		return err
	}
...
func resourceAliCloudPolarDbOnENSClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	d.Partial(true)
	polarDbServiceV2 := PolarDbServiceV2{client}
	...
	if d.HasChange("db_cluster_nodes_configs") {
		...
		addDebug("reconcileNodeAttributeFromThreeWay", nodeOperationPackage, d.Id())
	...
	if d.HasChange("cluster_version") {
		...
		if clusterVersion != objectRaw["DBLatestVersion"] {
			fmt.Errorf("must upgrade to the latest version %s", objectRaw["DBLatestVersion"])
		}
	...
	d.Partial(false)
	return resourceAliCloudPolarDbOnENSClusterRead(d, meta)
}

func resourceAliCloudPolarDbOnENSClusterDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	polarDbServiceV2 := PolarDbServiceV2{client}
...
func reconcileNodeAttributeFromThreeWay(d *schema.ResourceData, nodeAttributes *map[string]DBNodeAttribute, nodeConfigs *map[string]DBNodeConfig,
	...
	var err error
	nodeAttributes, err = mergeFromServerResponse(nodeAttributes, nodeConfigs, serverResponse, true)
	if err != nil {
		return nil, err
	}
...
func mergeNodeAttributeFromThreeWay(d *schema.ResourceData, nodeAttributes *map[string]DBNodeAttribute, nodeConfigs *map[string]DBNodeConfig,
	serverResponse *DescribeDBClusterAttributeResponse) error {
	var err error
	nodeAttributes, err = mergeFromServerResponse(nodeAttributes, nodeConfigs, serverResponse, false)
	if err != nil {
		return err
	}
...
func mergeFromServerResponse(nodeAttributes *map[string]DBNodeAttribute, nodeConfigs *map[string]DBNodeConfig,
	serverResponse *DescribeDBClusterAttributeResponse, reportOrphan bool) (*map[string]DBNodeAttribute, error) {
	for _, dbNode := range serverResponse.DBNodes {
		isCaught := false
		...
		for nodeKey, nodeConfig := range *nodeConfigs {
			...
			if nodeConfig.DBNodeClass == dbNode.DBNodeClass &&
				...
				addDebug("catch node "+nodeKey, nodeAttributes)
				break
			}
		}
		//orphan node
		if !isCaught && reportOrphan {
			addDebug("found orphan node "+dbNode.DBNodeId, nodeConfigs)
