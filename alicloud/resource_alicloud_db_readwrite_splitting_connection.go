	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"distribution_type": &schema.Schema{
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: validateAllowedStringValue([]string{string("Standard"), string("Custom")}),
			},
...
func resourceAlicloudDBReadWriteSplittingConnectionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	weight, ok := d.GetOk("weight")
	if ok && weight.(string) != "" {
		request.Weight = weight.(string)
	}
	...
	if err := resource.Retry(60*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedErrors(err, []string{"OperationDenied.ReadDBInstanceStatus"}) {
...
func resourceAlicloudDBReadWriteSplittingConnectionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("weight") {
		request.Weight = d.Get("weight").(string)
		d.SetPartial("weight")
		update = true
	}
