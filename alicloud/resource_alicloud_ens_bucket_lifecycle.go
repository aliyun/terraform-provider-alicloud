	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"created_before_date": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"expiration_days"},
			},
			"expiration_days": {
				Type:          schema.TypeInt,
				Optional:      true,
...
func resourceAliCloudEnsBucketLifecycleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if expiration, ok := objectRaw["Expiration"].(map[string]interface{}); ok {
		...
		if createdBefore, ok := expiration["CreatedBeforeDate"]; ok && createdBefore != nil {
			if cbStr := fmt.Sprint(createdBefore); cbStr != "" && cbStr != "<nil>" {
				d.Set("created_before_date", cbStr)
			}
		}
	}
