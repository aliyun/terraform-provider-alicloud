		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"rrsa_metadata.0.rrsa_oidc_issuer_url":   CHECKSET,
						"rrsa_metadata.0.ram_oidc_provider_name": CHECKSET,
						"rrsa_metadata.0.ram_oidc_provider_arn":  CHECKSET,
					}),
				),
			},
			{
				...
				ImportStateVerifyIgnore: []string{"name", "new_nat_gateway", "pod_cidr",
					...
					"cpu_policy", "proxy_mode", "cluster_domain", "custom_san", "node_port_range", "os_type", "platform", "timezone", "runtime", "taints", "encryption_provider_key", "rds_instances", "load_balancer_spec", "version", "rrsa_metadata"},
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"name":                name + "_update",
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"name", "new_nat_gateway", "pod_cidr",
					...
					"taints", "encryption_provider_key", "worker_data_disks", "rds_instances", "load_balancer_spec", "worker_number", "enable_rrsa", "rrsa_metadata"},
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"tags": map[string]string{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"name", "new_nat_gateway", "pod_cidr", "service_cidr", "control_plane_log_ttl",
					...
					"cpu_policy", "enable_ssh", "is_enterprise_security_group", "worker_number", "enable_rrsa", "node_port_range", "force_update", "rrsa_metadata",
				},
			},
		},
	})
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"name", "new_nat_gateway", "pod_cidr",
					...
					"worker_data_disk_category", "worker_data_disk_size", "master_vswitch_ids", "worker_vswitch_ids", "exclude_autoscaler_nodes",
					"cpu_policy", "proxy_mode", "cluster_domain", "custom_san", "node_port_range", "os_type", "platform", "timezone", "runtime", "taints", "encryption_provider_key", "rds_instances", "enable_rrsa", "rrsa_metadata"},
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"version": "1.24.3-aliyun.1",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"version": "1.24.3-aliyun.1",
